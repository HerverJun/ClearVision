param(
    ...
    [switch]$NoBuild,
    [switch]$NoRestore,
    [int]$MinimumEndpointTestClasses = 0,
...
if ($MinimumEndpointTestClasses -gt 0) {
    Write-Warning "MinimumEndpointTestClasses is retained for compatibility but no longer used; Suite=DesktopEndpoints is the fail-closed source."
}
$runner = Join-Path $PSScriptRoot "run-classified-test-gate.ps1"
$parameters = @{ Gate = "desktop-endpoints"; Verbosity = $Verbosity; ReturnExitCode = $true }
