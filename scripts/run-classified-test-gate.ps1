...
$validation = if ($null -ne $counters) {
    Test-TrxGreen -Counters $counters -RequiredTotal $effectiveMinimum
}
else {
    [ordered]@{
        completeGreen = $false
        issues = @("TRX counters are missing.")
        nonSuccessCounters = $null
    }
}

$sourceSha = (& git -C $repoRoot rev-parse HEAD 2>$null | Out-String).Trim()

$summary = [ordered]@{
    schemaVersion = "2026-08-02.classified-gate.v2"
    gate = $Gate
    sourceSha = $sourceSha
