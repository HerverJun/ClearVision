...
该算子用于任意角度旋转。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Angle` | 旋转角度 | `double` | 0 | [-360, 360] | Yes | - |
| `CenterX` | 中心X | `int` | -1 | - | Yes | - |
| `CenterY` | 中心Y | `int` | -1 | - | Yes | - |
| `Scale` | 缩放比例 | `double` | 1 | [0.1, 10] | Yes | - |
| `AutoResize` | 自动调整尺寸 | `bool` | true | - | Yes | - |
...
- 组合指纹 (Generation Fingerprint)：`C20D28781FD3D63738C1BD6FBC4EEF2913EA83720BC3479B9901107D5FD35787`
