...
| 标签 (Tags) | `分类:DefectDetection`, `分类显示:缺陷检测`, `生命周期:Stable`, `算法类型:自研` |

## 算法原理 / Algorithm Principle
该算子用于融合深度学习与传统视觉检测结果，输出最终判定。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `VotingStrategy` | Voting strategy | `enum` | WeightedAverage | WeightedAverage/Weighted average；Unanimous；Majority；PrioritizeDeepLearning/Prioritize deep learning；PrioritizeTraditional/Prioritize traditional | Yes | - |
