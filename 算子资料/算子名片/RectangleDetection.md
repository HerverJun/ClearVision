...
当前元数据描述为：Detects rectangular/quadrilateral objects from contours。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `MinArea` | Min Area | `int` | 100 | [0, 100000000] | Yes | - |
| `MaxArea` | Max Area | `int` | 10000000 | [0, 100000000] | Yes | - |
| `AngleTolerance` | Angle Tolerance | `double` | 15 | [0, 90] | Yes | - |
| `ApproxEpsilon` | Approx Epsilon | `double` | 0.02 | [0.0001, 1000] | Yes | - |
...
| `Height` | Height | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
