...
该算子用于支持腐蚀、膨胀、开运算、闭运算、梯度、顶帽、黑帽等形态学操作。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Operation` | 操作 | `enum` | Close | Erode/腐蚀；Dilate/膨胀；Open/开运算；Close/闭运算；Gradient/梯度；TopHat/顶帽；BlackHat/黑帽 | Yes | - |
| `KernelShape` | 核形状 | `enum` | Rect | Rect/矩形；Cross/十字；Ellipse/椭圆 | Yes | - |
| `KernelWidth` | 核宽度 | `int` | 3 | [1, 51] | Yes | - |
| `KernelHeight` | 核高度 | `int` | 3 | [1, 51] | Yes | - |
...
- 组合指纹 (Generation Fingerprint)：`5CC135A20B82BDF125E2069BDFE9C5D9B5959B7278E72612F27ED18B8CA8F0E3`
