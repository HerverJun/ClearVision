...
当前元数据描述为：Splits an image into tiled regions with optional overlap。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Rows` | Rows | `int` | 2 | [1, 100] | Yes | - |
| `Cols` | Cols | `int` | 2 | [1, 100] | Yes | - |
| `Overlap` | Overlap | `int` | 0 | [0, 10000] | Yes | - |
| `OutputMode` | Output Mode | `enum` | Array | Array/Array；Sequential/Sequential | Yes | - |
