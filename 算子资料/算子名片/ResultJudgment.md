...
| `Condition` | Condition | `enum` | Equal | Equal；NotEqual/Not Equal；GreaterThan/Greater Than；LessThan/Less Than；GreaterOrEqual/Greater Or Equal；LessOrEqual/Less Or Equal；Range | Yes | - |
| `ExpectValue` | Expected Value | `string` | 1 | - | Yes | - |
| `ExpectValueMin` | Expected Min | `string` | "" | - | Yes | - |
| `ExpectValueMax` | Expected Max | `string` | "" | - | Yes | - |
| `MinConfidence` | 最小置信度 | `double` | 0 | [0, 1] | Yes | - |
...
| `Confidence` | 置信度 | `Float` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
...
- 组合指纹 (Generation Fingerprint)：`7333F779253445A60AE8E15A9A9E2A76D962FE2A568320FA96C2C102CC4CCDEF`
