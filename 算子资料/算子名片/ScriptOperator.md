...
当前元数据描述为：Runs user-defined expression or script snippet。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
该类算子主要对上游值、集合或流程状态做判断、转换、聚合或路由，不直接改写图像像素。

## 实现策略 / Implementation Strategy
- 输入端口均为可选或该算子不依赖外部输入，执行时会优先读取可用输入并使用参数默认值兜底。
- 可选输入用于覆盖或补充参数配置：`Input1`、`Input2`、`Input3`、`Input4`。
...
|--------|------|------|--------|------|------|------|
| `ScriptLanguage` | Script Language | `enum` | CSharpExpression | CSharpExpression/CSharpExpression；CSharpScript/CSharpScript | Yes | - |
| `Code` | Code | `string` | Input1 + Input2 | - | Yes | - |
| `Timeout` | Timeout (ms) | `int` | 5000 | [1, 120000] | Yes | - |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `Input1` | Input 1 | `Any` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
| `Input2` | Input 2 | `Any` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
| `Input3` | Input 3 | `Any` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
| `Input4` | Input 4 | `Any` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Output1` | Output 1 | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `Output2` | Output 2 | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
2. 源码包含状态缓存或实例级状态，长流程运行时需要关注状态清理、并发调用和实例复用边界。
