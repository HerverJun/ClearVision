...
该算子用于使用指定结构元素对区域执行腐蚀（基于区域的形态学）。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|------|------|------|------|------|
| `Region` | 输入区域 | `Region` | Yes | 区域腐蚀的主输入，必须是 Region/像素区域；Image 或 Contour 不能直接替代。 |
| `Image` | 参考图像（可选） | `Image` | No | 仅用于参考图和结果可视化，不参与区域腐蚀计算，也不是主输入。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Region` | 腐蚀后区域 | `Region` | 腐蚀运算得到的 Region/像素区域。 |
...
- 组合指纹 (Generation Fingerprint)：`22190DF1942BBE6A61B9B17F8A39A885C3FA7CD1038ED52AB17128454C35B61F`
