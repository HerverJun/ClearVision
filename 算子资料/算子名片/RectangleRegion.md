...
该算子用于根据 X、Y、宽度和高度参数生成 Rectangle 矩形框，供需要 Rectangle 输入的算子使用。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `X` | X | `int` | 0 | >= 0 | Yes | - |
| `Y` | Y | `int` | 0 | >= 0 | Yes | - |
| `Width` | 宽度 | `int` | 1 | >= 1 | Yes | - |
| `Height` | 高度 | `int` | 1 | >= 1 | Yes | - |
...
|------|------|------|------|
| `Rectangle` | 矩形 | `Rectangle` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
- 组合指纹 (Generation Fingerprint)：`B0C81EEEAFB8BB1178453115CD4975BFFC33AD4FADFBC08F6588D2EF16BB220E`
