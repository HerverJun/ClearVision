...
- 先校验必填输入：`FeaturePrimary`、`DatumA`；缺失时通常返回失败结果。
- 可选输入用于覆盖或补充参数配置：`Image`、`DatumB`、`DatumC`。
...
|--------|------|------|--------|------|------|------|
| `ToleranceType` | Tolerance Type | `enum` | Parallelism | Parallelism/平行度；Perpendicularity/垂直度；Position/位置度；Concentricity/同心度 | Yes | - |
| `ZoneSize` | Zone Size | `double` | 2 | >= 0 | Yes | - |
| `EvaluationMode` | Evaluation Mode | `enum` | CircularZone | CircularZone/Circular Zone；RectangularZone/Rectangular Zone；Projected2D/Projected 2D | Yes | - |
| `NominalX` | Nominal X | `double` | 0 | - | Yes | - |
| `NominalY` | Nominal Y | `double` | 0 | - | Yes | - |
...
| `FeaturePrimary` | Primary Feature | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
| `DatumA` | Datum A | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
| `DatumB` | Datum B | `Any` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
| `DatumC` | Datum C | `Any` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Image` | 结果图像 | `Image` | 图像输出，可供后续图像处理、显示或保存节点使用。 |
| `Tolerance` | 公差带 | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `ZoneDeviation` | 偏离公差带 | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `AngularDeviationDeg` | 角度偏差(度) | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `LinearBand` | 线性偏差带(像素) | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
...
- 组合指纹 (Generation Fingerprint)：`818495EA9C4B17CB17AD464C165AE16C68C8AC232AD87FDD59DED58FED591FA1`
...
| `ToleranceMargin` | `Float` | 源码输出字典初始化中可见字段。 |
