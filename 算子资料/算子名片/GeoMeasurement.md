...
当前元数据描述为：General geometry measurement between point/line/circle elements。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Element1Type` | Element1 Type | `enum` | Auto | Auto/Auto；Point/Point；Line/Line；Circle/Circle | Yes | - |
| `Element2Type` | Element2 Type | `enum` | Auto | Auto/Auto；Point/Point；Line/Line；Circle/Circle | Yes | - |
