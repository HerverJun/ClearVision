...
| **亚像素级** | 99.75 | 200.35 | 100.6 | 100.6 | 示例坐标；误差未实测，不作固定精度承诺 |
...
| 应用场景 | 像素级误差 | 亚像素精度 |
|---------|-----------|-----------|
| 测量10mm零件，图像分辨率1000×1000 | 需测量 | 需以标定、成像与版本化 Benchmark 测量 |
...
**案例1：手机屏幕边框检测**
```
问题：测量手机边框宽度，业务要求由项目规格定义
图像：500万像素，视野50mm
分辨率：约0.01mm/像素

换算关系：像素误差 × 标定比例 = 物理量误差
结论：必须使用目标相机、镜头、标定和现场数据测量覆盖率与误差，不能由算法名称直接推导
```
