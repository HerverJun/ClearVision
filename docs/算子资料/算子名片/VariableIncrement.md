...
| `PreviousValue` | 前值 | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `NewValue` | 新值 | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `Delta` | 增量 | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `WasReset` | 是否已重置 | `Boolean` | 布尔判定结果，适合连接条件分支、结果判定或通信写入。 |
...
- 组合指纹 (Generation Fingerprint)：`E926ED16A6C9F3AD18C977B9D8A7E213AE087D64785B01D273B8F072221BAC6A`
