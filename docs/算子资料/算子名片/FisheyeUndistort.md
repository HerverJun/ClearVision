...
该算子用于使用标定数据校正鱼眼镜头畸变，并支持 LUT 加速。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `UseLut` | 使用LUT加速 | `bool` | true | - | Yes | - |
| `LutCacheSize` | LUT缓存大小 | `int` | 10 | [1, 100] | Yes | - |
| `Balance` | 平衡系数 | `double` | 0 | [0, 1] | Yes | - |
| `NewImageSizeFactor` | 新图像尺寸系数 | `double` | 1 | [0.1, 2] | Yes | - |
...
|------|------|------|------|
| `Image` | Undistorted Image | `Image` | 图像输出，可供后续图像处理、显示或保存节点使用。 |
| `UndistortedImage` | Undistorted Image | `Image` | 图像输出，可供后续图像处理、显示或保存节点使用。 |
...
- 组合指纹 (Generation Fingerprint)：`C1CD23B2ABC0ADC9E4583CAE66243585CA12AC92053A1414325ABFCC2BDAF9CF`
...
| `Applied` | `Any` | 源码通过输出字典索引赋值写入。 |
| `CalibrationKind` | `Float` | 源码通过输出字典索引赋值写入。 |
| `Diagnostics` | `Any` | 源码通过输出字典索引赋值写入。 |
| `DistortionModel` | `String` | 源码通过输出字典索引赋值写入。 |
| `Height` | `Integer` | 由图像输出封装自动附加，表示输出图像高度。 |
| `Message` | `String` | 源码通过输出字典索引赋值写入。 |
| `OriginalSize` | `Any` | 源码通过输出字典索引赋值写入。 |
| `OutputSize` | `Any` | 源码通过输出字典索引赋值写入。 |
| `UseLutAcceleration` | `Float` | 源码通过输出字典索引赋值写入。 |
