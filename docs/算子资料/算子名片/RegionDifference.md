...
当前元数据描述为：Computes the difference of two regions (A - B)。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|------|------|------|------|------|
| `Region1` | First Region (Minuend) | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
| `Region2` | Second Region (Subtrahend) | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Region` | Difference Region | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
