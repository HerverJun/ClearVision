...
该算子用于支持加减乘除、取绝对值、开方等常用运算。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Operation` | 运算类型 | `enum` | Add | Add/加 (+)；Subtract/减 (-)；Multiply/乘 (×)；Divide/除 (÷)；Abs/绝对值 (Abs)；Min/取小 (Min)；Max/取大 (Max)；Power/幂运算 (Power)；Sqrt/平方根 (Sqrt)；Round/取整 (Round)；Modulo/取余 (Modulo) | Yes | - |
...
|------|------|------|------|
| `Result` | 结果 | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `IsPositive` | 大于零 | `Boolean` | 布尔判定结果，适合连接条件分支、结果判定或通信写入。 |
...
- 组合指纹 (Generation Fingerprint)：`7199E66A395CF785F5E0436AC7CEA2D67F2A132FA18B4F02AFB85DD11FE03460`
- 显式共享依赖：无；指纹由最终运行时元数据与算子源码组成。

### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 推断类型 (Inferred Type) | 说明 (Description) |
|------|------|------|
| `InputA` | `Any` | 源码输出字典初始化中可见字段。 |
| `InputB` | `Any` | 源码输出字典初始化中可见字段。 |
| `IsNegative` | `Boolean` | 源码输出字典初始化中可见字段。 |
| `IsZero` | `Boolean` | 源码输出字典初始化中可见字段。 |
| `ResultFloat` | `Any` | 源码输出字典初始化中可见字段。 |
| `ResultInt` | `Any` | 源码输出字典初始化中可见字段。 |
