...
| 标签 (Tags) | `Compatibility`, `Deprecated`, `ImageOnly`, `Legacy`, `功能域:检测`, `成熟度:稳定`, `算法类型:自研` |

## 算法原理 / Algorithm Principle
当前元数据描述为：Legacy image morphology node. Use Morphological Operation for image workflows and Region* morphology operators for region workflows。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
