...
当前元数据描述为：Dilates a region using a specified structuring element (Region-based morphology)。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|------|------|------|------|
| `Region` | Dilated Region | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
