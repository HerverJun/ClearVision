...
当前元数据描述为：Stitches two images into a larger panorama-like output。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Method` | Method | `enum` | FeatureBased | FeatureBased/FeatureBased；Manual/Manual | Yes | - |
| `OverlapPercent` | Overlap Percent | `double` | 20 | [0, 90] | Yes | - |
| `BlendMode` | Blend Mode | `enum` | Linear | Linear/Linear；MultiBand/MultiBand | Yes | - |
