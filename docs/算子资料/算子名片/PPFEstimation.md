...
当前元数据描述为：Compute Point Pair Features (PPF) for a point cloud and build a per-point feature map。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `FeatureRadius` | Feature Radius | `double` | 0.05 | >= 1E-06 | Yes | - |
| `UseExistingNormals` | Use Existing Normals | `bool` | true | - | Yes | - |
