...
该算子用于从 CalibrationBundleV2 JSON 文件加载标定数据并输出强类型结果。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `CalibrationBundle` | Calibration Bundle | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
- 组合指纹 (Generation Fingerprint)：`E8896936329EC1949E20FF0C5B1710BD985499979FE30B977369B86558313306`
...
1. 外部文件、网络、PLC、数据库或消息系统不可用时，算子结果会受环境状态影响。
