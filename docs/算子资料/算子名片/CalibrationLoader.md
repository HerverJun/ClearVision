# Calibration Loader / CalibrationLoader
...
当前元数据描述为：Loads CalibrationBundleV2 JSON and exposes typed calibration outputs。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `FilePath` | Calibration File Path | `file` | "" | - | Yes | - |
