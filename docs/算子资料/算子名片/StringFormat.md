...
|--------|------|------|--------|------|------|------|
| `Mode` | 格式模式 | `enum` | Template | Template/模板；Join/拼接；Date/日期时间 | Yes | - |
...
| `Length` | 结果长度 | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `IsEmpty` | 结果为空 | `Boolean` | 布尔判定结果，适合连接条件分支、结果判定或通信写入。 |

## 模式与资源契约 / Mode & Resource Contracts
### 参数条件 / Parameter Conditions
| 参数 (Parameter) | 必填条件 (Required) | 可见条件 (Visible) | 启用/禁用条件 (Enabled/Disabled) | 忽略条件 (Ignored) | 资源 (Resource) | 输入可满足 (Satisfied By Inputs) | 原因码 (Reason) |
|------|------|------|------|------|------|------|------|
| `DateFormat` | metadata; - | visible: -; hidden: ALL(Mode != Date) | enabled: -; disabled: ALL(Mode != Date) | ALL(Mode != Date) | - | - | `STRING_FORMAT_DATE_MODE_ONLY` |
| `Separator` | metadata; - | visible: -; hidden: ALL(Mode != Join) | enabled: -; disabled: ALL(Mode != Join) | ALL(Mode != Join) | - | - | `STRING_FORMAT_JOIN_MODE_ONLY` |
| `Template` | metadata; - | visible: -; hidden: ALL(Mode != Template) | enabled: -; disabled: ALL(Mode != Template) | ALL(Mode != Template) | - | - | `STRING_FORMAT_TEMPLATE_MODE_ONLY` |
...
- 组合指纹 (Generation Fingerprint)：`8A52E735E09CD061D94CEBFA756218F1DFCF3ABCAFA9D1C5000D2C990BCDC9C1`
...
|------|------|----------|
| 1.0.0 | 2026-08-29 | 按当前最终运行时元数据、条件契约和显式依赖口径重生成 / Regenerated from effective runtime metadata and declared dependencies |
