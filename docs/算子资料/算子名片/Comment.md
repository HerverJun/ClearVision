...
> 中文：Reads the configured Text parameter, forwards the optional Input value unchanged, and increments image references when an image payload is passed through.。
> English: Reads the configured Text parameter, forwards the optional Input value unchanged, and increments image references when an image payload is passed through..
