...
|------|------|------|------|------|------|------|------|------|------|------|------|------|
| `Image` | Allowed:3, Rejected:1, Unknown:0 | Verified production support is present. | CV_8U | CV_8U | 1, 3, 4 | Verified 8-bit edge-detection inputs. | Convert to grayscale or model RGB without depth scaling. | 8-bit edge and visualization outputs. | Thresholds use the 0..255 intensity domain. | NotApplicableFor8U | `IMAGE_DEPTH_UNSUPPORTED` | `2.1` |

### 精确运行变体 / Exact Runtime Variants
| 输入端口 | 实际模式 | 精确输入类型（非笛卡尔积） | 条件 | 准入 | 验证 | 转换 | 输出 | 动态范围 | 输入值策略 | 失败码 | 证据 |
|------|------|------|------|------|------|------|------|------|------|------|------|
| `Image` | Default | CV_8UC1 | Canny and ONNX preprocessing accept verified 8-bit C1/C3/C4 images. | `Allowed` | `VerifiedSupport` | C3/C4 -> Gray for Canny; model preprocessing normalizes to three channels. | Edge mask is CV_8UC1 and visualization follows the operator output contract. | Legacy 0..255 threshold domain. | `Any` | `IMAGE_NONFINITE_INPUT` | `E2_OPERATOR_AND_PACKAGE_TESTS` |
| `Image` | Default | CV_8UC3, CV_8UC4 | Canny and ONNX preprocessing accept verified 8-bit C1/C3/C4 images. | `Allowed` | `VerifiedConversion` | C3/C4 -> Gray for Canny; model preprocessing normalizes to three channels. | Edge mask is CV_8UC1 and visualization follows the operator output contract. | Legacy 0..255 threshold domain. | `Any` | `IMAGE_NONFINITE_INPUT` | `E2_OPERATOR_AND_PACKAGE_TESTS` |
...
- 组合指纹 (Generation Fingerprint)：`7E0C2BA77CA2CB51AFB92141AE3CDDA821418CB5C70039572357B41D5E5218C7`
- `type:ClearVision.Product.Infrastructure.Operators.EdgeDetectionImageContractProvider`
