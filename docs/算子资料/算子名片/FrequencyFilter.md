...
该算子用于对一维或二维复数频谱执行频域滤波，用于保留或抑制指定频率成分。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
源码中包含 OpenCV 调用，核心处理通常围绕图像矩阵、ROI、阈值、几何计算或可视化结果图展开。

## 实现策略 / Implementation Strategy
- 先校验必填输入：`Spectrum`、`FilterType`；缺失时通常返回失败结果。
- 可选输入用于覆盖或补充参数配置：`CutoffLow`、`CutoffHigh`、`Order`。
...
|------|------|------|------|------|
| `Spectrum` | Input Frequency Spectrum | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
| `FilterType` | Filter Type | `String` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
| `CutoffLow` | Low Cutoff Frequency | `Float` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
| `CutoffHigh` | High Cutoff Frequency | `Float` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |
| `Order` | Filter Order | `Integer` | No | 可选输入；提供时会参与当前算子处理或覆盖部分参数配置。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `FilteredSpectrum` | Filtered Spectrum | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `FilterMask` | Filter Mask | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
- 组合指纹 (Generation Fingerprint)：`52A0030491EE19452768E854D58336B68736D1DCFAD56D2ADC50CAFA8FCA716D`
- 显式共享依赖：无；指纹由最终运行时元数据与算子源码组成。

### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 推断类型 (Inferred Type) | 说明 (Description) |
|------|------|------|
| `EffectiveCutoffHigh` | `Any` | 源码输出字典初始化中可见字段。 |
| `EffectiveCutoffLow` | `Any` | 源码输出字典初始化中可见字段。 |
| `Height` | `Integer` | 由图像输出封装自动附加，表示输出图像高度。 |
| `IsShifted` | `Boolean` | 源码输出字典初始化中可见字段。 |
| `ProcessingTimeMs` | `Any` | 源码输出字典初始化中可见字段。 |
| `SpectrumKind` | `Any` | 源码输出字典初始化中可见字段。 |
| `WasClamped` | `Any` | 源码输出字典初始化中可见字段。 |
