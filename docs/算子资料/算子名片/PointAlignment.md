...
- 先校验必填输入：`CurrentPoint`、`ReferencePoint`；缺失时通常返回失败结果。
...
|------|------|------|------|------|
| `CurrentPoint` | Current Point | `Point` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
...
- 组合指纹 (Generation Fingerprint)：`82EAB47A9674C349C9FCD49686035B85D16A088C34F4AA0696E766D22E7F9880`
