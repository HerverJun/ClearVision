...
当前元数据描述为：Corrects uneven illumination by background or model-based methods。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Method` | Method | `enum` | GaussianModel | DivideByBackground/DivideByBackground；GaussianModel/GaussianModel；MorphologicalTopHat/MorphologicalTopHat | Yes | - |
| `KernelSize` | Kernel Size | `int` | 51 | [3, 501] | Yes | - |
| `ColorMode` | Color Mode | `enum` | LumaOnly | LumaOnly/LumaOnly；PerChannel/PerChannel | Yes | - |
