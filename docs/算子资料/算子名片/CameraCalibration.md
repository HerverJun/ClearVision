...
当前元数据描述为：Calibrates camera intrinsics from chessboard or circle grid images。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `CalibrationOutputPath` | Calibration Output Path | `string` | calibration_result.json | - | Yes | - |
...
| `ReprojectionError` | `Any` | 源码通过输出字典索引赋值写入。 |
