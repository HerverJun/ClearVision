...
当前元数据描述为：Euclidean Cluster Extraction for point clouds (3D connected components by distance)。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `ClusterTolerance` | Cluster Tolerance | `double` | 0.02 | >= 1E-06 | Yes | - |
| `MinClusterSize` | Min Cluster Size | `int` | 100 | [1, 10000000] | Yes | - |
| `MaxClusterSize` | Max Cluster Size | `int` | 1000000 | [1, 10000000] | Yes | - |
...
|------|------|------|------|
| `Clusters` | Clusters | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `ClusterCount` | Cluster Count | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `ClusterPointClouds` | Cluster Point Clouds | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
