...
该算子用于一维码/二维码识别，支持 QR、Code128、DataMatrix 等多种码制。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `CodeType` | 码制类型 | `enum` | All | All/全部；QR/QR码；Code128；DataMatrix；EAN13/EAN-13；Code39 | Yes | - |
| `MaxResults` | 最大结果数 | `int` | 10 | [1, 100] | Yes | - |
...
| `Text` | 识别内容 | `String` | 文本结果，可用于显示、日志、保存或外部接口传输。 |
| `CodeCount` | 识别数量 | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `CodeType` | 条码类型 | `String` | 文本结果，可用于显示、日志、保存或外部接口传输。 |
...
- 组合指纹 (Generation Fingerprint)：`ABF6FC412AEAAA560886674DC3232D2A3F7F67CD5560E106477AAF840BAAD538`
- 显式共享依赖：无；指纹由最终运行时元数据与算子源码组成。

### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 推断类型 (Inferred Type) | 说明 (Description) |
|------|------|------|
| `CodeResults` | `Any` | 源码输出字典初始化中可见字段。 |
| `Codes` | `Any` | 源码输出字典初始化中可见字段。 |
| `Format` | `Any` | 源码输出字典初始化中可见字段。 |
| `Height` | `Integer` | 由图像输出封装自动附加，表示输出图像高度。 |
| `Index` | `Integer` | 源码输出字典初始化中可见字段。 |
| `Points` | `Any` | 源码输出字典初始化中可见字段。 |
| `ResultCount` | `Integer` | 源码输出字典初始化中可见字段。 |
