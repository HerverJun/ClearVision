...
当前元数据描述为：Composes multiple images by concat/grid/channel merge。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Mode` | Mode | `enum` | Horizontal | Horizontal/Horizontal；Vertical/Vertical；Grid/Grid；ChannelMerge/ChannelMerge | Yes | - |
| `Padding` | Padding | `int` | 0 | [0, 1000] | Yes | - |
| `BackgroundColor` | Background Color | `string` | #000000 | - | Yes | - |
