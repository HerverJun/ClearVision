...
该算子用于将图像切分为可选重叠的分块区域。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Rows` | 行数 | `int` | 2 | [1, 100] | Yes | - |
| `Cols` | 列数 | `int` | 2 | [1, 100] | Yes | - |
| `Overlap` | 重叠 | `int` | 0 | [0, 10000] | Yes | - |
| `OutputMode` | 输出模式 | `enum` | Array | Array/数组；Sequential/顺序 | Yes | - |
...
|------|------|------|------|
| `Tiles` | Tiles | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
- 组合指纹 (Generation Fingerprint)：`D96F87D16F84EB69B93D652ACBBC8766B2743F912F95249442C707B35DDF49A7`
