...
该算子用于从列表中按索引或条件提取元素。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
处理过程遵循统一算子框架：输入检查、参数解析、核心计算、输出封装和可选参数校验分层完成。

## 实现策略 / Implementation Strategy
- 先校验必填输入：`List`；缺失时通常返回失败结果。
...
|--------|------|------|--------|------|------|------|
| `Mode` | 提取模式 | `enum` | Index | Index/按索引；MaxConfidence/最大置信度；MaxArea/最大面积；MinArea/最小面积；First/第一个；Last/最后一个 | Yes | - |
| `Index` | 索引 | `int` | 0 | - | Yes | - |
| `LabelFilter` | 标签过滤 | `string` | "" | - | Yes | - |

## 输入/输出端口 / Input/Output Ports
### 输入 / Inputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 必填 (Required) | 说明 (Description) |
|------|------|------|------|------|
| `List` | 列表 | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Item` | 元素 | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `Found` | 是否找到 | `Boolean` | 布尔判定结果，适合连接条件分支、结果判定或通信写入。 |
| `Index` | 原始索引 | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
...
- 组合指纹 (Generation Fingerprint)：`B3BB274D288EDAB76422A67205BB5423BF82E65840BFC4B982A1DF65FA696DA1`
...
| `TotalCount` | `Integer` | 源码输出字典初始化中可见字段。 |
