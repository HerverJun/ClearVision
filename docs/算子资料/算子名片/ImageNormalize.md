...
该算子用于归一化像素分布，以提升下游处理稳定性。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Method` | 方法 | `enum` | MinMax | MinMax/最小最大；ZScore/Z分数；Histogram/直方图 | Yes | - |
| `Alpha` | Alpha | `double` | 0 | - | Yes | - |
| `Beta` | Beta系数 | `double` | 255 | - | Yes | - |
...
- 组合指纹 (Generation Fingerprint)：`0D788CB2E0E5DE7D7E952AFDA3BEB84969BA2AE93F67BC21D868CAEAF430C0A3`
