...
- 可选输入用于覆盖或补充参数配置：`ImageWidth`、`ImageHeight`、`Image`。
...
- 组合指纹 (Generation Fingerprint)：`ED7BA8434610266C5FA8278802918F5CA7B7BEFDF07F172E3600079ECCBDF309`
- 显式共享依赖：无；指纹由最终运行时元数据与算子源码组成。

### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 推断类型 (Inferred Type) | 说明 (Description) |
|------|------|------|
| `ClippedInputArea` | `Any` | 源码输出字典初始化中可见字段。 |
| `FillRatio` | `Float` | 源码输出字典初始化中可见字段。 |
| `Height` | `Integer` | 由图像输出封装自动附加，表示输出图像高度。 |
| `InputArea` | `Any` | 源码输出字典初始化中可见字段。 |
| `ProcessingTimeMs` | `Any` | 源码输出字典初始化中可见字段。 |
| `TotalArea` | `Any` | 源码输出字典初始化中可见字段。 |
