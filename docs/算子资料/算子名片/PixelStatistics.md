...
当前元数据描述为：Computes ROI/masked pixel-level statistics。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `Channel` | Channel | `enum` | Gray | Gray/Gray；R/R；G/G；B/B；All/All | Yes | - |
...
| `Median` | Median | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
