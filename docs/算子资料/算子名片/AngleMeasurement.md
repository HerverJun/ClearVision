...
该算子用于通过三点或两线计算角度，兼容亚像素级输入。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `Unit` | Angle Unit | `enum` | Degree | Degree；Radian | Yes | - |
...
- 组合指纹 (Generation Fingerprint)：`21FF978562E87C61B6B34C7F8BEDF33E9009A1F57A771EFAD853704130F5FA75`
