...
该算子用于根据轮廓检测矩形/四边形目标。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `MinArea` | 最小面积 | `int` | 100 | [0, 100000000] | Yes | - |
| `MaxArea` | 最大面积 | `int` | 10000000 | [0, 100000000] | Yes | - |
| `AngleTolerance` | 角度容差 | `double` | 15 | [0, 90] | Yes | - |
| `ApproxEpsilon` | 近似精度 | `double` | 0.02 | [0.0001, 1000] | Yes | - |
...
| `Rectangles` | Rectangles | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
| `Height` | 高度 | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `NormalizedAngle` | Normalized Angle | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `LongSide` | Long Side | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `ShortSide` | Short Side | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
...
- 组合指纹 (Generation Fingerprint)：`359E868BF0E1B2D203D0E7A9758BF3B63F6A7DB9E6DDED0B068604871053EFDE`
