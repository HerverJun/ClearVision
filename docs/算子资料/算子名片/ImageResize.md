...
该算子用于调整图像尺寸。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Width` | 目标宽度 | `int` | 640 | [1, 8192] | Yes | - |
| `Height` | 目标高度 | `int` | 480 | [1, 8192] | Yes | - |
| `ScaleFactor` | 缩放比例 | `double` | 1 | [0.01, 10] | Yes | - |
| `Interpolation` | 插值方法 | `enum` | Linear | Nearest/最近邻；Linear/双线性；Cubic/三次；Area/区域 | Yes | - |
| `UseScale` | 使用比例 | `bool` | false | - | Yes | - |
...
- 组合指纹 (Generation Fingerprint)：`2561198C2D09D75AFA6F2E547D20847B84826FD09D21B81BAA2F3024B65FE89A`
