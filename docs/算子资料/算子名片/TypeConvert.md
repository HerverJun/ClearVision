...
当前元数据描述为：Converts input data across String/Float/Integer/Boolean types。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|------|------|------|------|------|
| `Input` | Input | `Any` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Output` | Output | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `AsString` | As String | `String` | 文本结果，可用于显示、日志、保存或外部接口传输。 |
| `AsFloat` | As Float | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
