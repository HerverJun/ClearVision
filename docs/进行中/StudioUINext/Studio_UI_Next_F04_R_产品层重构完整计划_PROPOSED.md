...
> 当前状态：**COMPLETE / G4B FINAL EVIDENCE DONE**。
...
- Prompt 3 已完成 stable semantic sync、backend policy hardening 与黄金旅程工程候选，详见 [G3 工程候选与证据](./F04R_G3_黄金旅程工程候选与证据.md)。G4A.2 产品方向已获负责人确认，G4B 已完成真实 WebView2、Windows 125% DPI、Release publish、Remote CI 与 Final Gate；F04-R 已完成，但不自动改变默认入口、退役 Legacy 或启动 F05。
...
## 10.6 G4B 实际状态（2026-07-25）

真实宿主与发布证据见 [G4B WebView2 / DPI / Release 证据索引](./F04R_G4B_WebView2_DPI_Release证据索引.md)，最终阶段结论见 [F04-R 完成报告](./F04R_完成报告.md)。完整宿主矩阵的产品代码 SHA 为 `25f811c3cce697d4542fd59d4667804901f0e356`；后续 `5f3cf2ae...` 与 `70a0ab70...` 只包含 CI hygiene/source scan 最小修复，没有修改 Studio 产品运行时代码。Remote CI run `30143297252` 在正式默认配置下通过 Coverage Summary 与 Final Gate。
