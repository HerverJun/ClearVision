...
| F09-I004 | P2 | Workspace lifecycle evidence | F03 已在外部管理静态服务下得到代码候选 `5513f36c8750` 的 54/54 行为 PASS；受管 Playwright launcher 在断言通过后仍会因 Windows `taskkill /T /F` 返回 access denied 而无法自行结束。进程内 server 已增加连接关闭与有界退出兜底，仍需在可管理的执行环境复证 launcher clean exit/端口释放 | `f03-workspace.spec.ts`; `studio-ui-next-server.cjs`; 2026-08-04 外部 `CV_UI_BASE_URL` 运行 | known | G5 | open | 外部服务 F03 54/54 是有效行为证据；受管 teardown 是 P2 验证基础设施问题，不等同于已证实的 Workspace owner 泄漏。 |
...
| F09-I010 | P2 | Evidence integrity | 旧 evidence 的脏工作树和顶层 `-NoBuild` 缺口已由 runner 拒绝规则缓解：Profile、Rollback 与 Final runner 现在要求干净已提交工作树、首次使用 canonical fresh build，且拒绝 caller-supplied Desktop EXE。仍未证明未显式 Profile 覆盖的配置默认启动，因为 Profile/Final runner 为隔离场景仍注入 `Studio__StartupProfile` | `Invoke-StudioUiWebView2Evidence.ps1`; `Invoke-StudioUiProfileEvidence.ps1`; `Invoke-StudioUiFinalEvidence.ps1` | known | G5/G7 | open | 最终候选必须先提交，再用新鲜 build 复跑；Operator `/stations` 已加入场景，也须在该次复跑中验证。未覆盖启动仍保留为 P2，不能写成 `NEXT_RELEASE_STARTUP=PASS`。 |
