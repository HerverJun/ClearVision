...
- F09-R2：产品修复锚点为 source SHA `d1c82ba88e351a2d48bcfae7f97e047483dbba98`，后续产品源修复为 `c83dcc114290cf73e5e8d9b91e7b49732db8ec68` 与 `1545bca25`；当前本地证据运行 SHA 为 `9dd69bd2bde44e8ea5b7285bfd18f47e02f95007`。Profile、Rollback、Final、Browser、Product、Desktop 和 Release 125% 证据已完成，`ROLLBACK_DRILL=PASS`；审计分支 HEAD `06eddf63c488266f818bc36e1d14d6aa0f798333` 的 Remote CI run `30966530885` 因已提交 `ColorMeasurement=FAIL` measurement report 失败，Final Gate 同步失败，正式分支未推送。当前状态由 [F09 完成报告](./F09_完成报告.md)、[F09 Final Evidence Manifest](./F09_FinalEvidenceManifest.md)、[F09 Open Issues](./F09_OPEN_ISSUES.md) 和 [F09 Cutover/Rollback 手册](./F09_Cutover与Rollback操作手册.md) 维护：`F09_R_STATE=PARTIAL`、`F09_STATE=PARTIAL`、`FRONTEND_MIGRATION_MAINLINE=COMPLETE`、`CONFIGURED_PROFILE=NEXT_DEFAULT`、`EFFECTIVE_DEFAULT_UI_ROOT=STUDIO_UI_NEXT`、`LEGACY_ROLE=FALLBACK_ONLY`、`M_SERIES_ENTRY=BLOCKED_BY_REMOTE_CI`、`ROLLBACK_DRILL=PASS`、`REMOTE_CI=FAIL`、`FINAL_GATE=FAIL`、`PRODUCTION_ACCEPTANCE=NOT_GRANTED`。独立 no-Node、完整 DPI、现场硬件和生产 soak 保留为 acceptance debt。
- 命名 Profile `Studio:StartupProfile` 是当前 root/workspace 入口权威；`StudioUiEnabled` 等旧 raw flags 只作为配置投影输入。`LEGACY_FALLBACK` 仍可通过配置和重启恢复，不能与 Next 双写或同时 mounted。
