...
- F04：当前唯一执行权威是 [F04 完整开发计划](./Studio_UI_Next_F04_完整开发计划_PROPOSED_AUDITED.md)。G0/G1/G2/G3A/G3B/G3C 已完成；唯一 `projectLifecycleCommandOwner` 已接通 blank create、update、explicit open、revision-aware delete、unknown-outcome reconcile、Workspace leave protection 与 reauth。`G4_ENTRY=APPROVED`，G4–G6 尚未完成，必须继续串行。
