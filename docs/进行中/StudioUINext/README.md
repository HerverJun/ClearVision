...
- F09-R：工程主体已完成 Operator 只读投影、Profile 默认入口和 Desktop shutdown diagnostics 修复。当前状态由 [F09 完成报告](./F09_完成报告.md)、[F09 Final Evidence Manifest](./F09_FinalEvidenceManifest.md)、[F09 Open Issues](./F09_OPEN_ISSUES.md) 和 [F09 Cutover/Rollback 手册](./F09_Cutover与Rollback操作手册.md) 维护：`F09_R_STATE=ENGINEERING_DONE_WITH_ACCEPTANCE_DEBT`、`CONFIGURED_PROFILE=NEXT_DEFAULT`、`EFFECTIVE_DEFAULT_UI_ROOT=STUDIO_UI_NEXT`、`LEGACY_ROLE=FALLBACK_ONLY`、`OPERATOR_READONLY_UI_PROJECTION=PASS`、`M_SERIES_ENTRY=READY_WITH_ACCEPTANCE_DEBT`、`PRODUCTION_ACCEPTANCE=NOT_GRANTED`。
