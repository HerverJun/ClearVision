...
G05B 选择建立独立 Desktop-scoped `PreviewArtifactStore`，而不是复用或公开 `FlowExecutionService` 的 debug cache。代码依据是：`FlowExecutionService` 的 `_debugCache` 以 `(DebugSessionId, OperatorId)` 为执行复用键，配套 `_debugCacheFingerprints`、执行输入 fingerprint、变量上下文和 debug-session 清理语义；它属于 Infrastructure 执行复用缓存，不是可对前端公开读取的结果权威。Preview Artifact Store 只位于 Desktop 边界，owner identity 为 `ProjectId/TargetNodeId/DebugSessionId/ClientRequestSequence/FlowRevision`，只保存不可变 bytes 与 metadata，负责 transport projection、bearer artifactId 读取授权、TTL/容量/DELETE/owner revoke/dispose 生命周期和 WebView object URL 回收。若直接共享 debug cache，会把执行复用缓存变成公开 result authority，并把 fingerprint、变量上下文和复用命中语义泄漏给 UI，因此禁止。G05B 的 `/api/preview-artifacts/{artifactId}` 只接受 256-bit 以上 opaque token，返回 no-store blob；Observation envelope 只可携带可丢弃 `PreviewArtifactReferenceV1`，不得成为正式运行历史、ResultPath、Scene 或 Inspector authority。legacy preview 未传 `ArtifactMode` 时保留旧 Base64 兼容，此债务登记到 G16 收口删除。

G06 为 legacy Flow 页面新增受 Desktop 配置注入控制的 capability flag：`Studio:NodePreviewInspectorEnabled`，默认 `false`。Desktop `StudioOptions.NodePreviewInspectorEnabled` 是唯一配置权威，WebView2 启动注入发布不可替换的 `window.__CLEARVISION_STARTUP__`，并同时冻结外层 startup 与内层 `featureFlags`；composition root 在 bootstrap 时只读一次 `window.__CLEARVISION_STARTUP__.featureFlags["Studio:NodePreviewInspectorEnabled"]` 作为唯一 owner 决策来源。若保留顶层 `nodePreviewInspectorEnabled` 字段，仅作为只读诊断 alias，不参与决策；禁止 localStorage、查询参数、浏览器控制台或运行时热切换覆盖。flag off 时仅创建唯一 `NodePreviewCoordinator` 与旧 `NodePreviewOverlay`，不创建/注册 `NodePreviewInspector` 或 `nodePreviewSelectionStore`；flag on 时创建唯一 `NodePreviewInspector` 与 capability-local `nodePreviewSelectionStore`，不构造、不注册、不订阅旧 Overlay。两种路径复用同一个 `NodePreviewCoordinator`、`ExecutionObservationEnvelopeV1` 与 Preview Artifact client/resource bundle。rollback 为关闭该 flag 并重启页面回到旧 Overlay；cutover 条件为 G06 Inspector 的 Summary/Detail/Artifact、selectionStore、identity/stale、Artifact 有界读取、性能和守卫测试通过。后续 G15.2/G16 再迁移或删除旧 Preview capability，G06 不迁移正式 ResultPanel/history、不定义 Canonical ResultPath。
守卫必须防止：
