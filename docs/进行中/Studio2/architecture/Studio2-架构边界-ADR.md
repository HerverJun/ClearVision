...
G06 为 legacy Flow 页面新增受 Desktop 配置注入控制的 capability flag：`Studio:NodePreviewInspectorEnabled`，默认 `false`。capability owner 为 Observation / node-preview，composition root 在挂载前只读一次 `window.__CLEARVISION_STARTUP__.featureFlags`；禁止 localStorage、查询参数或浏览器控制台覆盖。flag off 时仅挂载旧 `NodePreviewOverlay`，flag on 时仅挂载 `NodePreviewInspector`，两种路径复用同一个 `NodePreviewCoordinator`、`ExecutionObservationEnvelopeV1` 与 Preview Artifact client/resource bundle。rollback 为关闭该 flag 回到旧 Overlay；cutover 条件为 G06 Inspector 的 Summary/Detail/Artifact、selectionStore、identity/stale、性能和守卫测试通过。后续 G15.2/G16 再迁移或删除旧 Preview capability，G06 不迁移正式 ResultPanel/history、不定义 Canonical ResultPath。
守卫必须防止：
