...
| Project | `projectManager` + legacy Project page | V2 Project page/API wrapper | `/api/projects/*`、Project DTO、`PersistenceRevision` | V2 typed API 包裹 `httpClient`，正式写入仍经 ProjectService + ProjectSaveCoordinator | G15.8 | G15.8 | G16 | `Studio2.Project` | 旧 Project 页面是唯一 owner | G04B 仅提供 `studio2.projectPersistencePort` 保存 facade，不迁移 Project list/page/create/delete/import/export；V2 Project 页面 owner 仍等待 G15.8 | flag off 恢复旧 Project 页面 | LOCKED |
