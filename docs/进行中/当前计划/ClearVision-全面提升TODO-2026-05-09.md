...
- [x] `pack.ps1 -RunSmokeTest` 与 CI 包 smoke 使用同一包版本和 locked restore 口径。
...
- [x] 增加轻量编码扫描脚本，至少检查 `U+FFFD replacement character`、常见 mojibake 片段和不可读中文。
...
- [x] 为 Station sync 表选择 EF migration 或明确 migration-light 机制，不能长期两套真相源。
- [x] 清理或标记 `Persistence/AppDbContext` 与实际 `Data/VisionDbContext` 的边界。
- 依据：`VisionDbContext.cs`、`VisionDatabaseInitializer.cs`、`VisionDatabaseMaintenance.cs`、`VisionDatabaseArchitectureGuardTests.cs`
...
- [x] 逐步减少 `ClearVision.Product.Runtime` 对 `Application` / `Infrastructure` 的直接引用。
