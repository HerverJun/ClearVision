...
- `ClearVision.Product/src/ClearVision.Product.Core/ValueObjects/Region.cs`
  - 基于 RLE 的 Region 表示。
  - 为避免与 OpenCvSharp 冲突，引入 `RegionRect` / `RegionPoint2f`。
- `ClearVision.Product/src/ClearVision.Product.Core/Enums/OperatorEnums.cs`
...
- `ClearVision.Product/src/ClearVision.Product.Infrastructure/Operators/LocalDeformableMatchingOperator.cs`
...
```powershell
dotnet build "ClearVision.Product.sln" -v minimal
```
...
```powershell
dotnet test "tests\ClearVision.Product.Tests\ClearVision.Product.Tests.csproj" --filter "FullyQualifiedName~Phase42RegionProcessingOperatorTests|FullyQualifiedName~Phase42MeasurementAndSignalOperatorTests|FullyQualifiedName~LocalDeformableMatchingPhase42Tests|FullyQualifiedName~PixelToWorldTransformOperatorTests|FullyQualifiedName~PlanarMatchingOperatorTests|FullyQualifiedName~ImageOutputLifecycleGuardTests" -v minimal
```
...
```powershell
dotnet test "tests\ClearVision.Product.Tests\ClearVision.Product.Tests.csproj" --filter "FullyQualifiedName~OperatorMetadataMigrationTests|FullyQualifiedName~OperatorContractReconciliationTests" -v minimal
```
