...
- [x] 给工站 `停止`、`部署`、`测试包下发` 增加风险确认和操作审计。
- [x] 恢复或明确结果追溯入口，区分当前页导出和全量导出。
- [x] 清理看板静态指标，未接入数据必须显示无数据或接口不可用。
- [x] 将开发者错误提示拆分为开发模式和现场模式。
- [x] 重命名并完善自动保存机制，区分本机草稿和正式工程保存。

### M1 近期体验改进

- [x] 算子库服务失败时不展示可误用默认算子。
- [x] 设置中心按现场角色和风险等级重组。
- [x] 存储路径、清理功能增加不可用原因和可执行替代步骤。
- [x] 线序模板未配置参数禁止运行、部署、发布。
- [x] 修复当前源码和部署脚本编码问题。
...
- [x] AI 建议应用前增加差异预览和撤销。
- [x] 首屏和空状态改为生产任务导向。
- [x] 建立统一现场中文术语表，并逐步替换混杂英文。

## 闭环记录

本计划已在 2026-05-20 完成本轮工程闭环。处理范围覆盖全部 P0、P1、P2 问题点：

- P0：工站高风险动作补二次确认和审计反馈；结果追溯入口恢复并接入结果页；导出增加数据源、范围、筛选、时间和数量元数据；看板未接入指标改为无数据或接口不可用；网络错误提示改为现场排障步骤；自动保存改为“本机草稿备份”并与正式保存明确区分。
- P1：算子库服务失败不再回退默认算子；设置中心入口和文案按现场语义重命名；存储路径保存前校验可访问性；线序视频模板对 ROI、模型、标签配置增加生产阻断声明和测试覆盖；源码日志和便携部署脚本编码风险已修复。
- P2：AI 应用动作明确为“应用到当前流程草稿”，保留差异预览和撤销；欢迎页与空状态改为生产任务导向；主要导航、标题、按钮、导入导出和工站监控提示完成现场中文术语收敛。

验证记录见归档目录 `docs/归档/已关闭事项/2026-05-20-当前计划闭环归档/闭环说明.md`。本计划归档后不再作为当前执行入口；后续若需要真实硬件、PLC、相机或产线节拍验证，应新建专项计划。
