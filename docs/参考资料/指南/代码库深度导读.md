...
> **覆盖范围**：`ClearVision.Product` 主产品、`ClearVision.PlcComm` 通信组件，以及与桌面宿主/前端/AI/标定/结果分析直接相关的主链路代码
...
- `ClearVision.Product/src/ClearVision.Product.Desktop`：桌面宿主、内置 Web 服务、API、WebView2 桥接。
- `ClearVision.Product/src/ClearVision.Product.Application`：应用服务层，如工程、检测、认证、用户管理、结果分析。
- `ClearVision.Product/src/ClearVision.Product.Infrastructure`：流程执行、图像采集、配置、AI、相机、算子元数据、预览等。
- `ClearVision.Product/src/ClearVision.Product.Core`：实体、值对象、接口、枚举、消息契约依赖。
- `ClearVision.Product/src/ClearVision.Product.Desktop/wwwroot/src`：前端视图层、画布、属性面板、检测页、设置页、AI 面板等。
- `ClearVision.Product/src/ClearVision.PlcComm`：PLC 通信协议实现。
...
- 通过分类树展示算子库。
- 当前仓库文档统计总算子数为 **156**，目录平均质量分为 **95.1** (`A=152 / B=4`).
- 支持关键字搜索、分类图标、参数面板编辑。
...
| 目录                                             | 角色       | 说明                                            |
| ------------------------------------------------ | ---------- | ----------------------------------------------- |
| `ClearVision.Product/src/ClearVision.Product.Desktop`        | 宿主层     | WinForms + WebView2 + 内嵌 ASP.NET Core Web API |
| `ClearVision.Product/src/ClearVision.Product.Application`    | 应用层     | 工程、检测、认证、结果分析、用户管理等业务服务  |
| `ClearVision.Product/src/ClearVision.Product.Infrastructure` | 基础设施层 | 流程执行、算子实现、AI、相机、配置、数据访问    |
| `ClearVision.Product/src/ClearVision.Product.Core`           | 领域层     | 实体、接口、枚举、值对象、核心抽象              |
| `ClearVision.Product/src/ClearVision.Product.Contracts`      | 契约层     | Web 消息模型、AI 生成消息模型                   |
| `ClearVision.Product/src/ClearVision.PlcComm`                | 通信组件   | PLC 协议客户端与地址/报文构建                   |
| `ClearVision.Product/tests`                           | 测试层     | 单元/集成/UI 测试                               |
...
- `ClearVision.Product/src` 下 C# 源文件数：**397**
- 前端 `wwwroot/src` 下 JS 文件数：**30**
- `算子资料/算子目录.md` 当前同步统计：算子总数 **156**，目录平均质量分 **95.1**.
...
### 8.16 PLC 通信组件：`ClearVision.PlcComm`
...
- `docs/参考资料/指南/用户指南.md`
- `docs/参考资料/指南/部署指南.md`
- `算子资料/算子目录.md`
- `算子资料/算子名片/*.md`
- `docs/参考资料/审计/当前Bug与架构隐患复核-2026-03-12.md`
- `docs/参考资料/历史资料/开发规划综合文档.md`
