...
**技术特点**：
- 自包含 .NET 8 运行时
- 单文件发布（ClearVision.Product.Desktop.exe）
...
```bash
# 从仓库根目录运行
& "./scripts/package-portable-deployment.ps1"
```
...
**默认输出位置**：`./.tmp/publish-check/portable-deployment/ClearVision-Portable-<yyyyMMdd-HHmmss>.zip`

如需生成正式交付物，显式传入仓库外目录，避免把临时发布产物写到仓库根目录：

```powershell
& "./scripts/package-portable-deployment.ps1" -OutputRoot "C:\ClearVisionRelease"
```
...
```bash
# 1. 发布 Release 版本
dotnet publish ClearVision.Product/src/ClearVision.Product.Desktop/ClearVision.Product.Desktop.csproj `
  -c Release `
  -r win-x64 `
  --self-contained true `
  ...
  -o ./.tmp/publish-check/ClearVision-1.0.0

# 2. 手动打包（可选）
Compress-Archive -Path ./.tmp/publish-check/ClearVision-1.0.0/* -DestinationPath ./.tmp/publish-check/ClearVision-1.0.0-win-x64.zip
```
...
3. **运行** 双击 `启动 ClearVision.bat` 或直接运行 `ClearVision.Product.Desktop.exe`
...
在 `ClearVision.Product/src/ClearVision.Product.Desktop.Package/Images/` 目录放置以下图标文件：
...
```bash
# 1. 还原包
dotnet restore ClearVision.Product/src/ClearVision.Product.Desktop.Package/ClearVision.Product.Desktop.Package.csproj

# 2. 发布主项目
dotnet publish ClearVision.Product/src/ClearVision.Product.Desktop/ClearVision.Product.Desktop.csproj -c Release

# 3. 构建 MSIX 包
dotnet build ClearVision.Product/src/ClearVision.Product.Desktop.Package/ClearVision.Product.Desktop.Package.csproj -c Release

# 4. 输出位置
# ClearVision.Product/src/ClearVision.Product.Desktop.Package/AppPackages/
```
...
```bash
# 1. 更新版本号（在代码中）
# 编辑 ClearVision.Product/src/ClearVision.Product.Desktop/ClearVision.Product.Desktop.csproj
...
### 1. 项目文件 (ClearVision.Product.Desktop.csproj)
...
# 环境变量
env:
  SOLUTION_PATH: ClearVision.Product/ClearVision.Product.sln
  DESKTOP_PROJECT: ClearVision.Product/src/ClearVision.Product.Desktop/ClearVision.Product.Desktop.csproj
```
...
```bash
# 创建占位图标目录
mkdir ClearVision.Product/src/ClearVision.Product.Desktop.Package/Images
# 放置所需图标文件（或从其他项目复制）
```
