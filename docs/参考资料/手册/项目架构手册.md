...
关键文件位置:
- `ClearVision.Product.Desktop/wwwroot/src/core/canvas/flowCanvas.js` - 核心画布实现
...
ClearVision.Product/
├── src/
│   ├── ClearVision.Product.Core/              # 🔵 领域核心层
│   │   ├── Entities/                   #    领域实体
│   │   │   ├── Operator.cs            #    算子实体定义
...
│   │
│   ├── ClearVision.Product.Application/       # 🟢 应用服务层
│   │   ├── DTOs/                       #    数据传输对象
│   │   │   ├── ProjectDto.cs          #    工程DTO
...
│   │   └── Services/                   #    应用服务
│   │
│   ├── ClearVision.Product.Contracts/         # 🟡 契约层(WebView2消息)
│   │   └── Messages/                   #    消息定义
│   │
│   ├── ClearVision.Product.Infrastructure/    # 🟠 基础设施层
│   │   ├── Operators/                  #    156个正式算子实现
...
│   │   └── Persistence/                #    数据持久化
│   │
│   └── ClearVision.Product.Desktop/           # 🔴 桌面宿主层
│       ├── wwwroot/                    #    前端资源
│       │   ├── src/
│       │   │   ├── core/canvas/       #    画布核心
...
**核心代码位置**: `ClearVision.Product.Infrastructure/Services/OperatorFactory.cs` (第90行起)
...
**新增算子需要修改的文件**:
1. `ClearVision.Product.Core/Enums/OperatorEnums.cs` - 添加新的算子类型枚举值
2. `ClearVision.Product.Infrastructure/Services/OperatorFactory.cs` - 添加算子元数据
3. `ClearVision.Product.Infrastructure/Operators/` - 创建算子执行器类（继承OperatorBase）
4. `ClearVision.Product.Desktop/wwwroot/src/features/operator-library/operatorLibrary.js` - 添加前端图标映射
...
定义位置: `ClearVision.Product.Core/Enums/OperatorEnums.cs` (第448-494行)
...
位置: `ClearVision.Product.Core/Enums/OperatorEnums.cs` (第12-334行)
...
[
  ...
  },
  {
    ...
    "params": [
      ...
      {
        "param_name": "LabelsPath",
        "display_name": "标签文件路径",
        "type": "file",
        "default": "",
        "description": "无 ONNX metadata names 时的后备标签文件路径；模型包含 metadata names 时忽略此项"
      }
    ],
...
**注**: 当前正式口径为 156 个算子；完整目录优先参考 `docs/算子资料/算子目录.json` 与根目录 `算子资料/算子目录.json`，运行时元数据以 `OperatorFactory` 和 `OperatorMetadataScanner` 输出为准。
...
序列化代码位置: `ClearVision.Product.Infrastructure/Services/ProjectJsonSerializer.cs`
...
存储位置: `ClearVision.Product.Infrastructure/Services/JsonFileProjectFlowStorage.cs`
...
```csharp
// 文件: ClearVision.Product.Infrastructure/Services/ProjectJsonSerializer.cs
public Task<ProjectDto?> DeserializeAsync(byte[] data)
{
    using var stream = new MemoryStream(data);
    var project = JsonSerializer.Deserialize<ProjectDto>(stream, _options);
...
| 用途 | 文件路径 |
|------|----------|
| 算子类型枚举 | `ClearVision.Product.Core/Enums/OperatorEnums.cs` |
| 算子元数据注册 | `ClearVision.Product.Infrastructure/Services/OperatorFactory.cs` |
| 算子执行器基类 | `ClearVision.Product.Infrastructure/Operators/OperatorBase.cs` |
| 流程执行服务 | `ClearVision.Product.Infrastructure/Services/FlowExecutionService.cs` |
| 工程序列化 | `ClearVision.Product.Infrastructure/Services/ProjectJsonSerializer.cs` |
| 前端画布 | `ClearVision.Product.Desktop/wwwroot/src/core/canvas/flowCanvas.js` |
| 工程DTO | `ClearVision.Product.Application/DTOs/ProjectDto.cs` |
| 流程DTO | `ClearVision.Product.Application/DTOs/OperatorFlowDto.cs` |
| 端口/参数值对象 | `ClearVision.Product.Core/ValueObjects/VisionValueObjects.cs` |
