...
当前元数据描述为：Computes subtraction or absolute difference between two images。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `AbsoluteDiff` | Absolute Diff | `bool` | true | - | Yes | Use absolute difference instead of signed subtraction. |
