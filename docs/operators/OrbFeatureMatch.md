...
当前元数据描述为：ORB feature matching with homography verification for fast template localization。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `CandidateProfile` | Candidate Profile | `enum` | default | default/Default；replay_safe_dense_strict/ORB replay_safe_dense_strict | Yes | - |
