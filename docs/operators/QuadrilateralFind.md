...
该算子用于查找不受直角约束的四边形轮廓。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `ConvexOnly` | 仅保留凸形 | `bool` | false | - | Yes | - |
...
| `Vertices` | Vertices | `PointList` | 点集结果，可连接几何测量、定位或标定相关节点。 |
| `OrderedVertices` | Ordered Vertices | `PointList` | 点集结果，可连接几何测量、定位或标定相关节点。 |
...
- 组合指纹 (Generation Fingerprint)：`086E5FB139C6DFA7DA205E778CC8698C894CDD6672AB7FA5CF8B09CC80DAB029`
