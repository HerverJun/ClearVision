...
当前元数据描述为：Runs non-maximum suppression on detection boxes。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `IouThreshold` | IoU Threshold | `double` | 0.45 | [0.1, 1] | Yes | - |
| `ScoreThreshold` | Score Threshold | `double` | 0.25 | [0, 1] | Yes | - |
| `MaxDetections` | Max Detections | `int` | 100 | [1, 1000] | Yes | - |
| `ShowSuppressed` | Show Suppressed | `bool` | true | - | Yes | - |
