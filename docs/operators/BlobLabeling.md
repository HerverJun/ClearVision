...
|--------|------|------|--------|------|------|------|
| `LabelBy` | 标注依据 | `enum` | Area | Area/面积；Circularity/圆度；AspectRatio/长宽比；Position/位置 | Yes | - |
| `Thresholds` | 阈值组 | `string` | [] | - | Yes | - |
| `DrawLabels` | 绘制标签 | `bool` | true | - | Yes | - |
...
| `Blobs` | Blob结果列表 | `BlobList` | No | BlobAnalysis.Blobs 输出的 Blob 结果列表；不是 Contour 或 Region。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Image` | Image | `Image` | 图像输出，可供后续图像处理、显示或保存节点使用。 |
| `Labels` | Labels | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
- 组合指纹 (Generation Fingerprint)：`FE849935D52A45C263D03B072708204D98C225C948B01BAC06AB9D5D17B35156`
...
| `Circularity` | `Any` | 源码输出字典初始化中可见字段。 |
| `Height` | `Integer` | 由图像输出封装自动附加，表示输出图像高度。 |
| `Index` | `Integer` | 源码输出字典初始化中可见字段。 |
| `Label` | `Any` | 源码输出字典初始化中可见字段。 |
