...
当前元数据描述为：Apply Gaussian filtering to suppress image noise。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
算法类型以 `Gaussian Blur (OpenCV)` 为主；元数据未声明更多细分时，以当前源码实现为准。
...
|--------|------|------|--------|------|------|------|
| `KernelSize` | Kernel Size | `int` | 5 | [1, 31] | Yes | - |
