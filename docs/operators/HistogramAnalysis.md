...
当前元数据描述为：Computes histogram and intensity-domain distribution statistics for a selected channel。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Channel` | Channel | `enum` | Gray | Gray/Gray；R/R；G/G；B/B | Yes | - |
| `BinCount` | Bin Count | `int` | 256 | [2, 1024] | Yes | - |
...
| `Mode` | Mode | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `Median` | Median | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `Peak` | Peak | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `Valley` | Valley | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `ModeBinIndex` | Mode Bin Index | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `MedianBinIndex` | Median Bin Index | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `PeakBinIndex` | Peak Bin Index | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `ValleyBinIndex` | Valley Bin Index | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |

### 运行时附加输出 / Runtime Additional Outputs
| 名称 (Name) | 推断类型 (Inferred Type) | 说明 (Description) |
|------|------|------|
| `BinWidth` | `Integer` | 源码输出字典初始化中可见字段。 |
