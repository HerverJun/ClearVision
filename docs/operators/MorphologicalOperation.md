...
当前元数据描述为：Erode, Dilate, Open, Close, Gradient, TopHat, BlackHat。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Operation` | Operation | `enum` | Close | Erode/Erode；Dilate/Dilate；Open/Open；Close/Close；Gradient/Gradient；TopHat/TopHat；BlackHat/BlackHat | Yes | - |
| `KernelShape` | Kernel Shape | `enum` | Rect | Rect/Rect；Cross/Cross；Ellipse/Ellipse | Yes | - |
| `KernelWidth` | Kernel Width | `int` | 3 | [1, 51] | Yes | - |
| `KernelHeight` | Kernel Height | `int` | 3 | [1, 51] | Yes | - |
