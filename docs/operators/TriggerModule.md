...
当前元数据描述为：Generates software, timer, or external triggers。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `TriggerMode` | Trigger Mode | `enum` | Software | Software/Software；Timer/Timer；ExternalSignal/ExternalSignal | Yes | - |
| `Interval` | Interval (ms) | `int` | 1000 | [1, 3600000] | Yes | - |
| `AutoRepeat` | Auto Repeat | `bool` | true | - | Yes | - |
