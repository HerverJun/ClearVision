...
该算子用于通过点或图像投影方式测量间距。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Direction` | 方向 | `enum` | Auto | Horizontal/水平；Vertical/垂直；Auto/自动 | Yes | - |
| `MinGap` | 最小间隙 | `double` | 0 | [0, 1000000] | Yes | - |
| `MaxGap` | 最大间隙 | `double` | 0 | [0, 1000000] | Yes | - |
| `ExpectedCount` | 期望数量 | `int` | 0 | [0, 10000] | Yes | - |
...
| `MinGap` | 最小间隙 | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
| `MaxGap` | 最大间隙 | `Float` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
...
- 组合指纹 (Generation Fingerprint)：`DF0DA993EE66D2A4C67723EAD4F9403C8DC601FC82A8ED867716D3096AA16710`
