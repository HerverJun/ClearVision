...
该算子用于计算两个区域的差集（A - B）。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
算法类型以 `Run-length row subtraction` 为主；元数据未声明更多细分时，以当前源码实现为准。
源码中包含 OpenCV 调用，核心处理通常围绕图像矩阵、ROI、阈值、几何计算或可视化结果图展开。

## 实现策略 / Implementation Strategy
- 先校验必填输入：`Region1`、`Region2`；缺失时通常返回失败结果。
...
|------|------|------|------|------|
| `Region1` | First Region (Minuend) | `Region` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |
| `Region2` | Second Region (Subtrahend) | `Region` | Yes | 必填输入，缺失时算子通常返回失败或无法产生有效结果。 |

### 输出 / Outputs
| 名称 (Name) | 显示名 (DisplayName) | 数据类型 (DataType) | 说明 (Description) |
|------|------|------|------|
| `Region` | Difference Region | `Region` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `Image` | Visualization | `Image` | 图像输出，可供后续图像处理、显示或保存节点使用。 |
| `Area` | Difference Area | `Integer` | 数值结果，可用于测量、阈值判定、统计或报表输出。 |
...
- 组合指纹 (Generation Fingerprint)：`718BCA61798413DD47C76F7771289D3877241567D51ABF5BB47B8F7C299084D8`
...
| `Region1Area` | `Any` | 源码输出字典初始化中可见字段。 |
| `Region2Area` | `Any` | 源码输出字典初始化中可见字段。 |
| `RemovedArea` | `Any` | 源码输出字典初始化中可见字段。 |
