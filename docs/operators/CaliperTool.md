...
| Evidence Identity | `clearvision-operator-quality-phase5-evidence@1.0.0:7ed1d600eead23e8185d6fef730b3995bec7db29c7d65a6cc3248a1ca95770cf/caliper-kernel-candidate-rejected` |
...
|--------|------|------|--------|------|------|------|
| `Direction` | 方向 | `enum` | Horizontal | Horizontal/水平；Vertical/垂直；Custom/自定义 | Yes | - |
| `Angle` | 角度 | `double` | 0 | [-180, 180] | Yes | - |
| `Polarity` | 极性 | `enum` | Both | DarkToLight/暗到亮；LightToDark/亮到暗；Both/双向 | Yes | - |
| `EdgeThreshold` | 边缘阈值 | `double` | 18 | [1, 255] | Yes | - |
| `ExpectedCount` | 期望数量 | `int` | 1 | [1, 100] | Yes | - |
| `MeasureMode` | 测量模式 | `enum` | edge_pairs | edge_pairs/边缘对 | Yes | - |
...
| `MeasurementEvidence` | Measurement Evidence | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
| `EdgeDiagnostics` | Edge Diagnostics | `Any` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
| `LocalizationSigmaPx` | `Any` | 源码通过输出字典索引赋值写入。 |
...
|------|------|----------|
| 1.2.1 | 2026-09-01 | 按当前最终运行时元数据、条件契约和显式依赖口径重生成 / Regenerated from effective runtime metadata and declared dependencies |
