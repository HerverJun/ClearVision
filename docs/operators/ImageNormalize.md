...
当前元数据描述为：Normalizes pixel distribution for robust downstream processing。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
|--------|------|------|--------|------|------|------|
| `Method` | Method | `enum` | MinMax | MinMax/MinMax；ZScore/ZScore；Histogram/Histogram | Yes | - |
| `Alpha` | Alpha | `double` | 0 | - | Yes | - |
| `Beta` | Beta | `double` | 255 | - | Yes | - |
