...
| 项目 (Field) | 值 (Value) |
|------|------|
| 类名 (Class) | `RectangleRegionOperator` |
| 枚举值 (Enum) | `OperatorType.RectangleRegion` |
| 分类 (Category) | 几何 |
| 版本 (Version) | `1.0.0` |
...
当前元数据描述为：Outputs a rectangular geometry value for operators that consume Rectangle input ports。运行时从声明输入端口读取数据，按参数表解析配置，并把处理结果写入输出字典。
...
| `X` | X | `int` | 0 | >= 0 | Yes | - |
| `Y` | Y | `int` | 0 | >= 0 | Yes | - |
| `Width` | Width | `int` | 1 | >= 1 | Yes | - |
| `Height` | Height | `int` | 1 | >= 1 | Yes | - |
...
|------|------|------|------|
| `Rectangle` | Rectangle | `Rectangle` | 业务输出字段，具体结构以源码输出和运行时结果为准。 |
...
|------|------|----------|
| 1.0.0 | 2026-07-10 | 按当前 `OperatorMetadataScanner` 口径重刷参数、端口、运行时附加输出、算法说明和限制 / Regenerated from current source metadata |
