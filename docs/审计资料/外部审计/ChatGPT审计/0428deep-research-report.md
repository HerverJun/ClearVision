...
| **可维护性**（依赖、复杂度） | 部分        | 中    | 采用.NET8和WinForms/SignalR等现代技术【77†L2-L10】；项目分解合理。依赖项数量适中，但`ClearVision.Product.Infrastructure`可能包罗过多责任。需评估单一项目复杂度、循环引用风险。 |
...
- **步骤：** 为每个算子模块新建测试项目（或扩充 `ClearVision.OperatorLibrary.SmokeTests`），编写测试用例；设置代码覆盖率工具（如 OpenCover）生成报告。
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Build
        run: dotnet build ClearVision.Product.sln --no-restore -c Debug
      - name: Test
        run: dotnet test ClearVision.Product.sln --no-build -c Debug
      - name: Publish
