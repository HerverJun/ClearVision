...
> - 产品优化方向二：做强“模板优先”，不要让 LLM 自由发挥  
> - 产品优化方向三：把算子库变成“AI 可理解的工业知识图谱”  
> - 产品优化方向四：把“用户输入”从一句话升级为需求澄清闭环  
> - 产品优化方向五：把 UX 从“聊天框”升级为“AI 工程师工作台”（重点）  
> - 产品优化方向八：工程架构与 LLM 策略优化
...
- [ ] 方向二：把现有模板匹配从线序关键词扩展为数据驱动的 `ScenarioMatcher`。
- [ ] 方向四：生成前新增 `RequirementBrief`，缺关键信息时先澄清，不急着调用 LLM。
- [ ] 方向五：把现有 `AiPanel` 的右侧结果区升级为工作台状态机，展示模板命中、缺资源、待确认参数、Validator、DryRun。
- [ ] 方向八：把 `AiFlowGenerationService` 中的生成链路拆成可测试 stage，但先不大规模重构外部 API。

### Phase 1：知识与评测闭环

目标：让 LLM 看到的不是“155 个算子大清单”，而是按场景检索出的工业知识切片。

- [ ] 方向三：生成 Operator Knowledge Graph，并让 PromptBuilder 使用相关算子子图。
- [ ] 方向二：建立模板 golden prompts 测试集与模板命中率评测。
- [ ] 方向四：把澄清问题、用户回答、参数补录写入会话上下文。
- [ ] 方向八：模型路由按任务角色拆分：意图识别、澄清、生成、修复、视觉附件理解。

### Phase 2：工程师工作台闭环

目标：让用户不只是“采纳一段 AI 结果”，而是完成一次可审查的视觉工程配置流程。

- [ ] 方向五：支持模板候选比较、生成结果 diff、分阶段应用、撤销、保存为模板、导出工程报告。
- [ ] 方向三：把质量矩阵证据、Known limitations、工业验证状态并入工作台风险提示。
- [ ] 方向八：离线/断网时进入模板向导降级模式，不阻塞用户配置高频场景。

---

# 方向二：做强“模板优先”，不要让 LLM 自由发挥
...
### P0-2.1：新增数据驱动的 ScenarioMatcher，替换硬编码关键词匹配
...
### P0-2.2：模板优先生成时，LLM 只能“补全”，不能随意改拓扑
...
### P0-2.3：增加 Template Gate，把模板约束接入 Validator
...
### P0-3.1：定义 Operator Knowledge Card JSON Schema
...
### P0-3.2：生成 Operator Knowledge Graph 边关系
...
### P0-3.3：PromptBuilder 改为“按场景取知识切片”
...
### P1-3.4：把知识图谱作为 Validator 与工作台的共同事实源
...
### P0-4.1：新增 RequirementBrief 数据结构
...
### P0-4.2：新增 ClarificationEngine，缺关键信息先问问题
...
### P1-4.4：把 PendingParameters / MissingResources 变成后置澄清闭环
...
# 方向五：把 UX 从“聊天框”升级为“AI 工程师工作台”（重点）
...
### P0-5.3：新增“需求卡片 + 缺口清单”工作台卡片

目标：把方向四的需求澄清在 UX 中落地。
...
### P0-5.5：新增 Validator / DryRun 控制台
...
### P0-8.1：把 AiFlowGenerationService 拆成阶段化 Pipeline
...
### P2-8.10：Scenario Package 版本化与资产校验
...
|---|---|---|---|---|
| P0 | ScenarioMatcher | 方向二 | 让模板优先从线序扩到全部内置高频模板 | AI 后端 |
| P0 | Template Lock + Template Gate | 方向二 | 限制 LLM 自由发挥，保证拓扑稳定 | AI 后端 / 质量 |
| P0 | RequirementBrief + ClarificationEngine | 方向四 | 生成前先补关键需求，减少无效生成 | AI 后端 / 产品 |
| P0 | AiWorkbench 状态机 | 方向五 | 让用户看懂生成阶段和结果状态 | 前端 |
| P0 | 模板匹配卡片 + 需求卡片 + 验证卡片 | 方向五 | 把现有字段变成可操作 UX | 前端 |
| P0 | Generation Pipeline 拆分 | 方向八 | 降低 `AiFlowGenerationService` 复杂度 | AI 后端 |
| P1 | Operator Knowledge Graph | 方向三 | 让 LLM 从算子清单升级为场景知识检索 | AI 后端 / 文档生成 |
| P1 | 模板评测集 | 方向二/八 | 用数据证明 Prompt 和模板策略稳定 | 质量 |
| P1 | 参数补录控件工程化 | 方向五 | 降低用户配置难度 | 前端 / 运行时 |
| P1 | 模型角色路由 | 方向八 | 降本增稳，视觉/生成/修复分工 | AI 后端 |
| P2 | 离线模板向导 | 方向八 | LLM 不可用时仍可落地高频场景 | 产品 / 前端 |
| P2 | 场景包版本化 | 方向二/八 | 模板从流程升级为工业资产包 | 架构 / 质量 |
