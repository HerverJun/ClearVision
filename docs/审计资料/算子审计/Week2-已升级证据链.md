...
| --- | --- | --- | --- | --- | --- | --- |
| [x] | Week2 C 线只处理 `AI检测(6)` 的验收补证，不重开 Week1 已审结的识别、标定和匹配定位事项。 | `docs/审计资料/算子审计/Week2-修复优先级清单.md`；`docs/审计资料/算子审计/Week1-审计周报.md` | 以 `Week2-长时启动计划.md` 的工作块C为唯一执行入口。 | 无 | C线负责人（证据与复核） | 2026-04-20 |
| [x] | Week2 内 AI 检测只有同时具备 `批次结果 + 模型版本 + 阈值 + 环境 + 工业签收` 时，才允许转 `[x]`。 | `docs/审计资料/算子审计/启动计划/Week2-长时启动计划.md`；`docs/审计资料/算子审计/审计执行标准.md` | 检查本文件第 2-4 节是否同时具备上述 5 类信息。 | 无 | C线负责人（证据与复核） | 2026-04-20 |

## 2. AI检测验收补证包

| 类别 | 当前口径 | 文档证据 | 自动化/回归入口 | 当前结论 | 缺口 |
| --- | --- | --- | --- | --- | --- |
| AI检测（6） | Week1 已绑定 `detection-all-20260413-115808.trx`，Week2 需要补齐工业签收、模型版本、阈值和环境一致性。 | `docs/审计资料/算子审计/Week1-已升级证据链.md`；`docs/审计资料/算子审计/Week2-修复优先级清单.md` | `scripts/run-tests-detection-regression.ps1 -Gate all` | [-] 已于 2026-04-13 12:52（UTC+8）启动 Week2 补证，当前先沿用 Week1 `detection-all` 批次作为初始批次。 | 工业签收记录、模型版本、阈值说明、环境说明待绑定。 |
...
| 模型 | `models/model_catalog.json` + `ClearVision.Product/tests/ClearVision.Product.Tests/Operators/*` | 第 5 节模板：`模型版本` | AI检测 Owner（模型） | `2026-04-13 19:43（UTC+8，已回填可得项）` | 补齐“模型标识 + 版本 + 唯一标识”，并与批次绑定；生产模型唯一标识由签收附件承接 | 先升级至 C2 负责人；若仍缺失，升级至 C线负责人（证据与复核） |
| 阈值 | `ClearVision.Product/tests/ClearVision.Product.Tests/Operators/*`（参数口径） | 第 5 节模板：`阈值` | AI检测 Owner（阈值） | `2026-04-13 19:43（UTC+8，已回填可得项）` | 补齐“配置阈值 + 运行时阈值”，并与批次绑定 | 先升级至 C2 负责人；若仍缺失，升级至 C线负责人（证据与复核） |
...
| 2026-04-13（周一） | Week4 C4 收口发布与 Week5 入口 | [x] | 已发布 Week4 结论，生成 `docs/审计资料/算子审计/Week5-入口卡片.md` 并绑定唯一缺口。 |
