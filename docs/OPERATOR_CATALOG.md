...
| `OperatorType.MorphologicalOperation` | 形态学操作 | 1 | 1 | 7 | 94 (A) | `1.0.0` | 当前元数据描述为：Erode, Dilate, Open, Close, Gradi… | [MorphologicalOperation](./operators/MorphologicalOperation.md) |
...
| `OperatorType.Thresholding` | 全局阈值处理 | 1 | 1 | 4 | 94 (A) | `1.0.0` | 当前元数据描述为：Global thresholding with optional… | [Thresholding](./operators/Thresholding.md) |
...
| `OperatorType.FFT1D` | 信号/图像傅里叶变换（FFT） | 2 | 4 | 0 | 89 (A) | `1.0.0` | 当前元数据描述为：Performs FFT on 1D signals and em… | [FFT1D](./operators/FFT1D.md) |
...
| `OperatorType.FrequencyFilter` | 频域滤波 | 5 | 3 | 0 | 81 (B) | `1.0.0` | 当前元数据描述为：Applies frequency-domain filters … | [FrequencyFilter](./operators/FrequencyFilter.md) |
| `OperatorType.HistogramEqualization` | 直方图均衡化 | 1 | 1 | 4 | 94 (A) | `1.0.0` | 当前元数据描述为：Supports global histogram equaliz… | [HistogramEqualization](./operators/HistogramEqualization.md) |
...
| `OperatorType.ImageSubtract` | 图像减法 | 2 | 4 | 1 | 89 (A) | `1.0.0` | 当前元数据描述为：Computes subtraction or absolute … | [ImageSubtract](./operators/ImageSubtract.md) |
...
| `OperatorType.InverseFFT1D` | 信号/图像逆傅里叶变换（IFFT） | 2 | 4 | 0 | 89 (A) | `1.0.0` | 当前元数据描述为：Performs inverse FFT on 1D spectr… | [InverseFFT1D](./operators/InverseFFT1D.md) |
...
| `OperatorType.RoiManager` | ROI裁剪与掩膜 | 1 | 3 | 10 | 100 (A) | `1.0.0` | 该算子用于矩形/圆形/多边形区域选择。运行时从声明输入端口读取数据，按参数表解析配置… | [RoiManager](./operators/RoiManager.md) |
...
| `OperatorType.BoxFilter` | 候选框筛选 | 2 | 3 | 9 | 94 (A) | `1.0.0` | 当前元数据描述为：Filters detections by area, class… | [BoxFilter](./operators/BoxFilter.md) |
| `OperatorType.BoxNms` | 候选框抑制 | 3 | 7 | 4 | 90 (A) | `1.0.0` | 当前元数据描述为：Runs non-maximum suppression on d… | [BoxNms](./operators/BoxNms.md) |
...
| `OperatorType.Statistics` | 统计分析 | 1 | 7 | 5 | 90 (A) | `1.0.0` | 当前元数据描述为：Computes Mean/StdDev/Cpk statisti… | [Statistics](./operators/Statistics.md) |
...
| `OperatorType.TimerStatistics` | 计时统计 | 1 | 4 | 4 | 94 (A) | `1.0.1` | 当前元数据描述为：Measures elapsed and cycle time s… | [TimerStatistics](./operators/TimerStatistics.md) |
| `OperatorType.TypeConvert` | 类型转换 | 1 | 6 | 2 | 100 (A) | `1.0.0` | 当前元数据描述为：Converts input data across String… | [TypeConvert](./operators/TypeConvert.md) |
...
| 枚举 (Enum) | 显示名 (DisplayName) | 输入 | 输出 | 参数 | 质量 (Q) | 版本 (Version) | 算法 (Algorithm) | 文档 |
|------|------|------:|------:|------:|------|------|------|------|
| `OperatorType.CalibrationLoader` | 标定加载 | 0 | 3 | 1 | 100 (A) | `1.0.0` | 当前元数据描述为：Loads CalibrationBundleV2 JSON an… | [CalibrationLoader](./operators/CalibrationLoader.md) |
| `OperatorType.CameraCalibration` | 相机标定 | 1 | 2 | 7 | 100 (A) | `1.0.0` | 当前元数据描述为：Calibrates camera intrinsics from… | [CameraCalibration](./operators/CameraCalibration.md) |
| `OperatorType.CoordinateTransform` | 像素到物理坐标（单点） | 4 | 3 | 2 | 90 (A) | `1.0.0` | 当前元数据描述为：Converts pixel coordinates to phy… | [CoordinateTransform](./operators/CoordinateTransform.md) |
| `OperatorType.FisheyeCalibration` | 鱼眼标定 | 1 | 2 | 9 | 100 (A) | `1.0.0` | 当前元数据描述为：Calibrates fisheye camera intrins… | [FisheyeCalibration](./operators/FisheyeCalibration.md) |
| `OperatorType.FisheyeUndistort` | 鱼眼去畸变 | 2 | 2 | 4 | 96 (A) | `1.0.0` | 当前元数据描述为：Correct fisheye lens distortion u… | [FisheyeUndistort](./operators/FisheyeUndistort.md) |
...
| `OperatorType.NPointCalibration` | N点标定 | 1 | 9 | 10 | 100 (A) | `1.0.0` | 当前元数据描述为：Builds robust affine or homograph… | [NPointCalibration](./operators/NPointCalibration.md) |
| `OperatorType.PixelToWorldTransform` | 像素世界映射 | 3 | 3 | 11 | 100 (A) | `1.0.1` | 当前元数据描述为：Transforms coordinates via Calibr… | [PixelToWorldTransform](./operators/PixelToWorldTransform.md) |
| `OperatorType.StereoCalibration` | 双目标定 | 2 | 6 | 11 | 100 (A) | `1.0.0` | 当前元数据描述为：Calibrates stereo camera pair and… | [StereoCalibration](./operators/StereoCalibration.md) |
...
| `OperatorType.Undistort` | 畸变校正 | 2 | 1 | 0 | 91 (A) | `1.0.0` | 当前元数据描述为：Correct lens distortion using cal… | [Undistort](./operators/Undistort.md) |

### 检测 (17)
| 枚举 (Enum) | 显示名 (DisplayName) | 输入 | 输出 | 参数 | 质量 (Q) | 版本 (Version) | 算法 (Algorithm) | 文档 |
|------|------|------:|------:|------:|------|------|------|------|
| `OperatorType.AngleMeasurement` | 角度测量 | 6 | 3 | 7 | 96 (A) | `1.0.0` | 当前元数据描述为：Measures angle from three points … | [AngleMeasurement](./operators/AngleMeasurement.md) |
...
| `OperatorType.GeometricFitting` | 几何拟合 | 1 | 2 | 8 | 100 (A) | `1.0.0` | 当前元数据描述为：Fits line, circle or ellipse from… | [GeometricFitting](./operators/GeometricFitting.md) |
...
| `OperatorType.PointLineDistance` | 点线距离 | 2 | 2 | 2 | 96 (A) | `1.0.0` | 当前元数据描述为：Computes distance from a point to… | [PointLineDistance](./operators/PointLineDistance.md) |
| `OperatorType.WidthMeasurement` | 宽度测量 | 3 | 8 | 8 | 96 (A) | `1.0.0` | 当前元数据描述为：Measures width between parallel e… | [WidthMeasurement](./operators/WidthMeasurement.md) |
...
| `OperatorType.ResultJudgment` | 结果判定 | 2 | 5 | 8 | 90 (A) | `1.0.1` | 当前元数据描述为：Generic business judgment with nu… | [ResultJudgment](./operators/ResultJudgment.md) |
...
| `OperatorType.TryCatch` | Try分支透传 | 1 | 4 | 3 | 93 (A) | `1.0.0` | 该算子用于Try-Catch 流程控制。运行时从声明输入端口读取数据，按参数表解析配… | [TryCatch](./operators/TryCatch.md) |
...
| `OperatorType.EdgeDetection` | 边缘检测 | 1 | 2 | 14 | 100 (A) | `1.0.0` | 当前元数据描述为：Detects edges with Canny and opti… | [EdgeDetection](./operators/EdgeDetection.md) |
...
| `OperatorType.SharpnessEvaluation` | 清晰度评估 | 1 | 3 | 8 | 96 (A) | `1.0.0` | 当前元数据描述为：Evaluates focus quality of an ima… | [SharpnessEvaluation](./operators/SharpnessEvaluation.md) |
| `OperatorType.SubpixelEdgeDetection` | 亚像素边缘 | 1 | 2 | 5 | 94 (A) | `1.0.0` | 当前元数据描述为：Non-industrial reference subpixel… | [SubpixelEdgeDetection](./operators/SubpixelEdgeDetection.md) |
...
| `OperatorType.TextSave` | 文本保存 | 2 | 2 | 5 | 100 (A) | `1.0.0` | 当前元数据描述为：Saves text or structured data to … | [TextSave](./operators/TextSave.md) |
...
| `OperatorType.MitsubishiMcCommunication` | 三菱MC通信 | 1 | 2 | 13 | 98 (A) | `1.0.0` | 当前元数据描述为：Mitsubishi MC protocol PLC read/w… | [MitsubishiMcCommunication](./operators/MitsubishiMcCommunication.md) |
| `OperatorType.ModbusCommunication` | Modbus TCP通信 | 1 | 2 | 9 | 100 (A) | `1.0.0` | 当前元数据描述为：Industrial Modbus TCP communicati… | [ModbusCommunication](./operators/ModbusCommunication.md) |
| `OperatorType.MqttPublish` | MQTT 发布 | 2 | 1 | 6 | 100 (A) | `0.1.0` | 当前元数据描述为：Publishes inspection data to MQTT… | [MqttPublish](./operators/MqttPublish.md) |
