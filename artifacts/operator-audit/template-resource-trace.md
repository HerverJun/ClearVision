...
| Legacy Agent acceptance | `ClearVision.Product/src/ClearVision.Product.Infrastructure/AI/Tools/VisionAgentParameterRuleCenter.cs:105-119` | Agent validation treats any of `Template`, `TemplateId`, or `TemplatePath` as sufficient metadata. This is a readiness rule, not a runtime conversion. |
...
| Shared legacy parity | `quality/evals/specs/vision_agent_parameter_rule_parity_cases.json` | TemplateMatching cases remain separate from the five provider-backed operators and document metadata acceptance versus the actual missing `Template` resource. |
