...
export function normalizeWorkspaceSnapshotForRestore(raw) {
    ...
    const snapshot = {
        ...
        readinessPreview: trusted ? (read(raw, 'readinessPreview') || null) : null,
        ...
        buildRunId: trusted ? String(read(raw, 'buildRunId') || '').trim() : '',
        buildRunStatus: trusted ? safeRunStatus(read(raw, 'buildRunStatus')) : 'idle',
        buildTerminalSequence: trusted ? finiteNonNegative(read(raw, 'buildTerminalSequence'), null) : null,
        submittedBuildFingerprint: trusted ? String(read(raw, 'submittedBuildFingerprint') || '').trim() : '',
        trusted,
        degraded: !trusted,
        appliedDowngraded,
