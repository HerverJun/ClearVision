...
const scopeTimeLabel = computed(() => {
  ...
  return `${formatDateTime(scope.startTime, '最早')} 至 ${formatDateTime(scope.endTime, '现在')}`;
});
    ...
    >
      ...
      >
        ...
        <dl>
          ...
          <div>
            ...
            <dd>
              <code
                v-if="owner.projection.scope.diagnosticCode"
                translate="no"
              >{{ scopeDiagnosticLabel }}</code>
              <span v-else>{{ scopeDiagnosticLabel }}</span>
            </dd>
          </div>
        </dl>
        <p class="results-export-dialog__scope-note">
          结果文件包含服务端当前工程历史查询能读取的全部匹配记录，不受页面分页大小影响。
        </p>
        <p
          v-if="owner.projection.scope.diagnosticCode"
          class="results-export-dialog__scope-note results-export-dialog__scope-note--diagnostic"
        >
          页面内诊断码只筛选当前页表格；本次导出会由服务端将该诊断码作为完整导出范围条件。
        </p>
      </section>

      <div class="results-export-dialog__control-row">
        <CvSelect
          v-model="formatModel"
      ...
      >
        ...
        <dl>
          ...
          <div><dt>结果文件</dt><dd><code translate="no">{{ owner.projection.job.fileName }}</code></dd></div>
          <div><dt>结果截止时间</dt><dd>{{ formatDateTime(owner.projection.job.snapshotUpperBoundUtc, '创建任务时') }}</dd></div>
...
.results-export-dialog__section-heading > span {
  color: var(--cv-text-muted);
  font-size: var(--cv-font-size-xs);
}

.results-export-dialog dl {
...
.results-export-dialog dl div {
  min-width: 0;
  padding: var(--cv-space-2) 0;
  border-bottom: 1px solid var(--cv-border-subtle);
}

.results-export-dialog dt {
  color: var(--cv-text-muted);
  font-size: var(--cv-font-size-xs);
}

.results-export-dialog dd {
  margin: 2px 0 0;
...
.results-export-dialog__scope-note {
  max-width: 72ch;
  margin: var(--cv-space-2) 0 0;
...
.results-export-dialog__scope-note--diagnostic {
  color: var(--cv-color-status-info-strong);
}

.results-export-dialog__control-row {
  display: grid;
...
.results-export-dialog__status {
  min-height: calc(var(--cv-density-control-height) + var(--cv-density-field-gap) + var(--cv-font-size-xs));
  display: flex;
  align-items: center;
  gap: var(--cv-space-2);
}

.results-export-dialog__status p {
  min-width: 0;
  margin: 0;
  color: inherit;
  font-size: var(--cv-font-size-xs);
...
@media (max-width: 560px) {
  .results-export-dialog__scope { padding-inline: var(--cv-space-3); }
  .results-export-dialog__control-row,
