...
[OperatorMeta(
    DisplayName = "区域腐蚀",
    Description = "使用指定结构元素对区域执行腐蚀（基于区域的形态学）。",
...
[InputPort("Region", "输入区域", PortDataType.Region, IsRequired = true, Description = "区域腐蚀的主输入，必须是 Region/像素区域；Image 或 Contour 不能直接替代。")]
[InputPort("Image", "参考图像（可选）", PortDataType.Image, IsRequired = false, Description = "仅用于参考图和结果可视化，不参与区域腐蚀计算，也不是主输入。")]
[OutputPort("Region", "腐蚀后区域", PortDataType.Region, Description = "腐蚀运算得到的 Region/像素区域。")]
