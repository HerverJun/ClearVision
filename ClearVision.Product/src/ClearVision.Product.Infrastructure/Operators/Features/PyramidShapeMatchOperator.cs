...
public class PyramidShapeMatchOperator : OperatorBase
{
    ...
    private static (ClearVision.Product.Infrastructure.ImageProcessing.ShapeMatchResult primary, List<ClearVision.Product.Infrastructure.ImageProcessing.ShapeMatchResult> all) DrawShapeMatchResults(
        Mat image,
        List<ClearVision.Product.Infrastructure.ImageProcessing.ShapeMatchResult> matches,
        float minScore)
    {
        var validMatches = matches.Where(m => m.IsValid).ToList();
        var primaryMatch = validMatches.FirstOrDefault() ?? new ClearVision.Product.Infrastructure.ImageProcessing.ShapeMatchResult { IsValid = false };
