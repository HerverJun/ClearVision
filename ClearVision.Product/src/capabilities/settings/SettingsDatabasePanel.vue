...
async function loadStatus(): Promise<void> {
  ...
  phase.value = 'loading';
  message.value = null;
  const result = await props.owner.readDatabaseStatus();
...
async function createBackup(): Promise<void> {
  if (!isAdmin || backupBusy.value) return;
  if (!window.confirm('确认创建当前数据库备份？本轮不会提供 restore、cleanup 或 repair。')) return;
  backupBusy.value = true;
  feedback.value = null;
  backup.value = null;
  try {
    const result = await props.owner.backupDatabase();
    ...
    <template v-else>
      ...
      <template v-if="status">
        <div class="settings-database__heading">
          ...
          <CvStatusBadge :tone="statusTone(status.state)" :label="status.state" />
        </div>
      ...
      <CvInlineAlert class="settings-database__deferred" tone="info" title="延期操作">
        restore、cleanup、repair 和全局 reset 本轮没有入口，也不会由前端直接操作 SQLite。
      </CvInlineAlert>
    </template>
  </CvPanel>

  <div v-if="feedback" class="settings-database__feedback">
    <CvInlineAlert
