...
[Collection(PerformanceAcceptanceCollection.Name)]
    ...
    public class PerformanceAcceptanceTests
    {
        ...
        private OperatorFlow CreateComplexFlow()
        {
            ...
            var inputNode = new Operator("ImageAcquisition", ClearVision.Product.Core.Enums.OperatorType.ImageAcquisition, 0, 0);
            inputNode.AddOutputPort("Image", ClearVision.Product.Core.Enums.PortDataType.Image);

            var matchNode = new Operator("TemplateMatch", ClearVision.Product.Core.Enums.OperatorType.TemplateMatching, 200, 0);
            matchNode.AddInputPort("Input", ClearVision.Product.Core.Enums.PortDataType.Image, true);
            matchNode.AddOutputPort("MatchScore", ClearVision.Product.Core.Enums.PortDataType.Float);

            var outputNode = new Operator("ResultOutput", ClearVision.Product.Core.Enums.OperatorType.ResultOutput, 400, 0);
            outputNode.AddInputPort("Input1", ClearVision.Product.Core.Enums.PortDataType.Any, true);
        ...
        public async Task DeepStability_6000x4000_WithFanOut_ShouldMaintainPoolHitRateAndP99()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                var blurNode = new Operator($"Blur_{i}", ClearVision.Product.Core.Enums.OperatorType.MedianBlur, 200, i * 150);
                blurNode.AddInputPort("Image", ClearVision.Product.Core.Enums.PortDataType.Image, true);
                blurNode.AddOutputPort("OutputImage", ClearVision.Product.Core.Enums.PortDataType.Image);
                blurNode.AddParameter(new Parameter(Guid.NewGuid(), "KernelSize", "KernelSize", "", "int", 3));
                flow.AddOperator(blurNode);

                var cropNode = new Operator($"Crop_{i}", ClearVision.Product.Core.Enums.OperatorType.ImageCrop, 400, i * 150);
                cropNode.AddInputPort("Image", ClearVision.Product.Core.Enums.PortDataType.Image, true);
                cropNode.AddOutputPort("OutputImage", ClearVision.Product.Core.Enums.PortDataType.Image);
        ...
        public async Task ForEach_ParallelMode_ShouldExecuteInParallel()
        {
            ...
            var foreachNode = new Operator("ForEach", ClearVision.Product.Core.Enums.OperatorType.ForEach, 0, 0);
            ...
            var delayNode = new Operator("Delay", ClearVision.Product.Core.Enums.OperatorType.ResultOutput, 0, 0);
