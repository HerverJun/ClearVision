...
internal static class PreprocessingTestSupport
{
    ...
    {
        var candidate = Path.Combine(ResolveWorkspaceRoot(), "ClearVision.Product", "tests", "TestData", relativePath);
    ...
    public static string EnsureReportDirectory(string? reportDirectoryEnvName = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(path))
        {
            path = Path.Combine(ResolveWorkspaceRoot(), "ClearVision.Product", "test_results");
        }
