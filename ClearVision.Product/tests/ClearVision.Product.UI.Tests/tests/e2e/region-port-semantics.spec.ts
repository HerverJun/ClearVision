...
test('legacy BlobAnalysis flow ports migrate during real canvas deserialization', async ({ page }) => {
  await stubOperatorLibrary(page);
  await bootAuthenticatedApp(page);
  await expect(page.locator('#flow-canvas')).toBeVisible();
  ...
  const migrated = await page.evaluate(() => {
    ...
    canvas.deserialize({
      operators: [
        {
          id: 'legacy-blob-analysis',
          type: 'BlobAnalysis',
          name: '旧版 Blob 分析',
          x: 20,
          y: 20,
          inputPorts: [{ id: 'legacy-image', name: 'Image', dataType: 'Image', isRequired: true }],
          outputPorts: [
            { id: 'legacy-blobs', name: 'Blobs', dataType: 'Contour' },
            { id: 'legacy-features', name: 'BlobFeatures', dataType: 'Any' },
          ],
          parameters: [],
        },
        {
          id: 'legacy-blob-labeling',
          type: 'BlobLabeling',
          name: '旧版 Blob 标注',
          x: 420,
          y: 20,
          inputPorts: [{ id: 'legacy-label-blobs', name: 'Blobs', dataType: 'Contour' }],
          outputPorts: [],
          parameters: [],
        },
      ],
    ...
    const analysis = canvas.nodes.get('legacy-blob-analysis');
    const labeling = canvas.nodes.get('legacy-blob-labeling');
    return {
      blobs: analysis.outputs.find((port: any) => port.name === 'Blobs')?.dataType,
      blobFeatures: analysis.outputs.find((port: any) => port.name === 'BlobFeatures')?.dataType,
      labelingBlobs: labeling.inputs.find((port: any) => port.name === 'Blobs')?.dataType,
    };
  });

  expect(migrated).toEqual({
    blobs: 'BlobList',
    blobFeatures: 'BlobFeatureList',
    labelingBlobs: 'BlobList',
  });
});
