...
process.env.CV_UI_PORT = process.env.CV_UI_PORT?.trim() || '5319';
...
export default defineConfig({
  ...
  forbidOnly: true,
  reporter: [['list']],
  outputDir: resolve(uiTestsRoot, '../../../.tmp/studio-ui-next/r2/baseline-playwright-results'),
  use: {
  ...
  projects: [{ name: 'r2-baseline-chromium', use: { ...devices['Desktop Chrome'] } }]
});
