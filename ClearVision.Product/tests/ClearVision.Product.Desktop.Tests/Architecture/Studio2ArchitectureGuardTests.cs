...
public sealed class Studio2ArchitectureGuardTests
{
    ...
    [Fact]
    public void LegacyFlowCanvasAdapter_ShouldKeepCanonicalHostedFactory()
    {
        ...
        Regex.Matches(legacyAdapterText, @"\bnew\s+FlowCanvas\s*\(")
            .Should().ContainSingle("the canonical hosted adapter must create exactly one FlowCanvas instance.");
    }
