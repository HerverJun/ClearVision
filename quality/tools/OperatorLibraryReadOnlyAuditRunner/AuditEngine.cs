...
public static class AuditSerialization
{
    ...
    public static string SerializeMarkdown(AuditReport report)
    {
        var lines = new List<string>
        {
            ...
            $"| Static confirmed findings | {report.Summary.ConfirmedCount} |",
            ...
            $"| Accepted intentional differences | {report.Summary.Review.IntentionalDifferenceCount} |",
            $"| Audit false positives | {report.Summary.Review.AuditFalsePositiveCount} |",
            $"| Accepted confirmed baseline identities | {report.Summary.ConfirmedBaselineCount} |",
            $"| New confirmed gate | {(report.Summary.NewConfirmedCount == 0 ? "pass" : "fail")} ({report.Summary.NewConfirmedCount}) |",
            string.Empty,
            "## Audit surfaces",
            string.Empty,
        ...
        [
            ...
            $"- New confirmed gate: {(report.NewConfirmedFindings.Count == 0 ? "pass" : "fail")} ({report.NewConfirmedFindings.Count})",
            string.Empty
        ]);
