...
var result = ContractBatch2Runner.Run();
...
Console.WriteLine(
    $"G1 P3 contract batch2 complete: {result.Summary.Passed}/{result.Summary.CaseCount} passed, " +
...
internal static class ContractBatch2Runner
{
    ...
    [
        new(OperatorType.BoxNms, typeof(BoxNmsOperator), "flow-data-priority"),
        new(OperatorType.ConditionalBranch, typeof(ConditionalBranchOperator), "flow-data-priority"),
        new(OperatorType.PointSetTool, typeof(PointSetToolOperator), "flow-data-priority"),
        new(OperatorType.Statistics, typeof(StatisticsOperator), "flow-data-priority"),
        new(OperatorType.TriggerModule, typeof(TriggerModuleOperator), "flow-data-priority"),
        new(OperatorType.TryCatch, typeof(TryCatchOperator), "flow-data-priority"),
        new(OperatorType.Aggregator, typeof(AggregatorOperator), "flow-data-priority"),
        new(OperatorType.PPFEstimation, typeof(PPFEstimationOperator), "point-cloud-3d"),
        new(OperatorType.PPFMatch, typeof(PPFMatchOperator), "point-cloud-3d"),
        new(OperatorType.RansacPlaneSegmentation, typeof(RansacPlaneSegmentationOperator), "point-cloud-3d"),
        new(OperatorType.StatisticalOutlierRemoval, typeof(StatisticalOutlierRemovalOperator), "point-cloud-3d"),
        new(OperatorType.VoxelDownsample, typeof(VoxelDownsampleOperator), "point-cloud-3d"),
        new(OperatorType.PointAlignment, typeof(PointAlignmentOperator), "measurement-geometry"),
        new(OperatorType.PointCorrection, typeof(PointCorrectionOperator), "measurement-geometry"),
        new(OperatorType.PointLineDistance, typeof(PointLineDistanceOperator), "measurement-geometry"),
        new(OperatorType.RoiTransform, typeof(RoiTransformOperator), "measurement-geometry"),
        new(OperatorType.ShadingCorrection, typeof(ShadingCorrectionOperator), "image-preprocessing"),
        new(OperatorType.SharpnessEvaluation, typeof(SharpnessEvaluationOperator), "image-preprocessing"),
        new(OperatorType.UnitConvert, typeof(UnitConvertOperator), "flow-data-priority"),
        new(OperatorType.ResultOutput, typeof(ResultOutputOperator), "io-mock-contract"),
        new(OperatorType.CodeRecognition, typeof(CodeRecognitionOperator), "io-mock-contract"),
        new(OperatorType.DatabaseWrite, typeof(DatabaseWriteOperator), "io-mock-contract", ("ConnectionString", "")),
        new(OperatorType.EuclideanClusterExtraction, typeof(EuclideanClusterExtractionOperator), "point-cloud-3d"),
        new(OperatorType.GlcmTexture, typeof(GlcmTextureOperator), "texture"),
        new(OperatorType.HttpRequest, typeof(HttpRequestOperator), "io-mock-contract", ("Url", "")),
        new(OperatorType.ImageAdd, typeof(ImageAddOperator), "image-preprocessing"),
        new(OperatorType.ImageSave, typeof(ImageSaveOperator), "io-mock-contract"),
        new(OperatorType.LawsTextureFilter, typeof(LawsTextureFilterOperator), "texture"),
        new(OperatorType.MinEnclosingGeometry, typeof(MinEnclosingGeometryOperator), "measurement-geometry"),
        new(OperatorType.ModbusCommunication, typeof(ModbusCommunicationOperator), "io-mock-contract", ("IpAddress", "")),
        new(OperatorType.MqttPublish, typeof(MqttPublishOperator), "io-mock-contract", ("Topic", "")),
        new(OperatorType.OcrRecognition, typeof(OcrRecognitionOperator), "io-mock-contract"),
        new(OperatorType.PolarUnwrap, typeof(PolarUnwrapOperator), "image-preprocessing"),
        new(OperatorType.RoiManager, typeof(RoiManagerOperator), "measurement-geometry"),
        new(OperatorType.ScriptOperator, typeof(ScriptOperator), "flow-data-priority", ("Script", "")),
        new(OperatorType.SerialCommunication, typeof(SerialCommunicationOperator), "io-mock-contract", ("PortName", "")),
        new(OperatorType.TcpCommunication, typeof(TcpCommunicationOperator), "io-mock-contract", ("Host", ""), ("IpAddress", "")),
        new(OperatorType.TextSave, typeof(TextSaveOperator), "io-mock-contract", ("FilePath", ""))
    ];

    public static BaselineResult Run()
    {
        var matrixRows = LoadMatrixRows(MatrixPath);
        var cases = new List<ContractCase>(Specs.Length * 21);
    ...
    private static void AddSpecCases(List<ContractCase> cases, OperatorSpec spec, IReadOnlyDictionary<string, MatrixRow> matrixRows)
    {
        ...
        {
            var executor = CreateExecutor(spec.ImplementationType);
            var result = executor.ExecuteAsync(CreateOperator(spec.OperatorType, spec.SafeParameters), null).GetAwaiter().GetResult();
            DisposeIfNeeded(executor);
        ...
        Add(cases, spec, "matrix_level_a", "Matrix registry contract", () =>
        {
            ...
            Require(row.Level == "A", "Batch2 operators should stay in level A scope.");
        ...
        Add(cases, spec, "card_sections_present", "Card contract", () =>
        {
            ...
            var hasAlgorithmOrSemantics = text.Contains("算法原理", StringComparison.Ordinal)
                ...
                || text.Contains("算子语义", StringComparison.Ordinal)
        ...
        Add(cases, spec, "batch2_group_frozen", "Batch selection contract", () =>
        {
            Require(
                spec.Group is "flow-data-priority"
                    or "io-mock-contract"
                    or "point-cloud-3d"
                    or "measurement-geometry"
                    or "image-preprocessing"
                    or "texture",
                "Batch2 group should be frozen to the approved priority lanes.");
    ...
    private static bool CanResolve(Type parameterType)
    {
        return parameterType == typeof(IServiceProvider)
            ...
            || parameterType == typeof(OcrEngineProvider)
    ...
    private static object Resolve(Type parameterType)
    {
        ...
        if (parameterType == typeof(OcrEngineProvider))
        {
            return new OcrEngineProvider(NullLogger<OcrEngineProvider>.Instance);
        }
    ...
    {
        var op = new Operator(Guid.NewGuid(), $"{type}G1P3ContractBatch2", type, 0, 0);
...
internal sealed record OperatorSpec(
    ...
    string Group,
    params (string Name, object Value)[] SafeParameters);
...
internal static class MarkdownReport
{
    public static string Create(BaselineResult result)
    {
        ...
        {
            "# G1 P3 Contract Batch 2 Baseline",
            "",
    ...
    string? ParseError)
{
    ...
    {
        var output = "quality/evals/reports/G1P3ContractBatch2_baseline.json";
        string? report = "quality/evals/reports/G1P3ContractBatch2_baseline.md";
