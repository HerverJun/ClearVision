...
$operatorsRoot = Join-Path $repoRoot "ClearVision.Product/src/ClearVision.Product.Infrastructure/Operators"
...
if ($outputRoot -ne $scriptRootFull -and -not $outputRoot.StartsWith($scriptRootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "OutputDir must resolve inside ClearVision.OperatorLibrary: $outputRoot"
}
...
$usingMatches = Select-String `
    ...
    -Pattern "^\s*using\s+(ClearVision\.Product\.(Core|Infrastructure)\.[A-Za-z0-9_.]+)\s*;" `
    -AllMatches
...
[void]$md.AppendLine('- This report is generated from `ClearVision.Product.Infrastructure/Operators/*.cs`.')
...
[void]$md.AppendLine('- For Phase 3.3, use this report together with abstraction adapters under `ClearVision.OperatorLibrary/src`.')
